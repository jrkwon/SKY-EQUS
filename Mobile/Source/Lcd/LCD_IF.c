...
{
    (void) outp(LCD_BASE_ADDR, lcd_command);
} /* end of hs_lcd_out */
...
void	LCDCommandDataEnable ( void )
{
	// STB low
#ifdef SURF_WS
	BIO_OUT( ENC_GPIO_0, BIO_LCD_STB_M, BIO_LCD_STB_LO_V );
#else
	BIO_OUT( DEC_GPIO_1, BIO_LCD_STB_M, BIO_LCD_STB_LO_V );
#endif
}
...
void	LCDCommandDataDisable ( void )
{
	// STB high
#ifdef SURF_WS
	BIO_OUT( ENC_GPIO_0, BIO_LCD_STB_M, BIO_LCD_STB_HI_V );
#else
	BIO_OUT( DEC_GPIO_1, BIO_LCD_STB_M, BIO_LCD_STB_HI_V );
#endif
}

void hs_lcd_icon(byte nSeg, byte data)
{
	LCDCommandDataEnable ();
	hs_lcd_out(LCD_COMMAND_ICON_ADDRESS_SET);
	hs_lcd_out(nSeg/8);
	hs_lcd_out(0);
