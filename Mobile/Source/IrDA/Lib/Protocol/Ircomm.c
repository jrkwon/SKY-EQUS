...
/* $Header: IRCOMM.CV  0.1  96/07/14 00:00:00  core  Exp $ */
...
#define		COMMMAX		IrCOMM_LSAPMAX
#define		COMMRXSDUSZ	IrCOMM_RXSDUSZ
#define		COMMTXSDUSZ	IrCOMM_TXSDUSZ

#define		RXQUEUESZ	IrCOMM_RXQUESZ
#define		TXQUEUESZ	IrCOMM_TXQUESZ
#define		CTLQUEUESZ	IrCOMM_CTLQUESZ

//#include	"pircomm.h"		for my compiler
// this code is pircomm.h
...
#if !defined(COMMMAX)
#error	COMMMAXÇ™ê≥ÇµÇ≠Ç†ÇËÇ‹ÇπÇÒÅBégópÇ∑ÇÈÉTÅ[ÉrÉXêîÅi0x01 < n < 0x80ÅjÇíËã`ÇµÇƒâ∫Ç≥Ç¢ÅB
#endif
#if !defined(COMMTXSDUSZ)
#error	COMMTXSDUSZÇ™ê≥ÇµÇ≠Ç†ÇËÇ‹ÇπÇÒÅBëóêMàÍéûÉoÉbÉtÉ@ÇÃÉTÉCÉYÇíËã`ÇµÇƒâ∫Ç≥Ç¢ÅB
#endif
#if !defined(CTLQUEUESZ)
#error	CTLQUEUESZÇ™ê≥ÇµÇ≠Ç†ÇËÇ‹ÇπÇÒÅBLSAPñàÇÃëóêMêßå‰ÉRÅ[ÉhÉLÉÖÅ[ÇÃÉTÉCÉYÇíËã`ÇµÇƒâ∫Ç≥Ç¢ÅB
#endif
...
int		IrDAAPI	IrCOMM_Open( void )
{
	...
	for( i=0 ; i<COMMMAX ; i++ ) {
		cc		= &COMMConfig[i] ;
		IrDA_memset( (void*)&cc->flags, 0, sizeof( COMMFLAGS )) ;
...
/*
   // tasik 01.03.23
	lsap∞° ¡ˆΩ√«œ¥¬ LSAP∏¶ ∞≥πÊ«—¥Ÿ. ¡Ô, lsap∞° ¡¢º”µ«æÓ ¿÷¥Ÿ∏È
	∞≠¡¶∑Œ ≈ÎΩ≈¿ª ≤˜æÓπˆ∏Æ¥¬ ¿œ¿ª «—¥Ÿ.
*/
...
static	void	commTTP_RecvData( int lsap )
{
	...
	for(;;) {
		...
		ptr		= (const byte	IrDAFAR *)TTP_GetDataPtr( lsap ) ;
		if( len > 1 ) {
			ctlsz	= *ptr ;
			datsz	= len - 1 - ctlsz ;
			ctlp	= &ptr[1] ;
