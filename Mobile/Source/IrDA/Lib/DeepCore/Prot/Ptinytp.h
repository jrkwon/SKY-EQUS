...
int		IrDAAPI	TTP_Open( void )
{
	...
	for( i=0 ; i<TTPMAX ; i++ ) {
		TTPConfig[i].Mode	= 0 ;
		IrDA_memset( &TTPConfig[i].flags, 0, sizeof( TTPFLAGS )) ;
	}
...
int		IrDAAPI	TTP_BindLSAP( int tsap, TTPCALLBACK IrDAFAR *ttpc )
{
	...
	IrDA_memcpy( &TTPConfig[tsap].CallBack, ttpc, sizeof( TTPCALLBACK )) ;
...
uint	IrDAAPI	TTP_GetRxSize( int lsap )
{
	...
	if(( tc->Mode & TTPM_Connected ) == 0 )
		return( 0 ) ;
	if( ttpGetCredit( &tc->RemoteCredit ) == tc->RxSduTblSize )
		return( 0 ) ;
...
#endif	
{
	...
	return( &tc->RxSduData[tc->RxSduReadPtr*tc->RxSduElmSize] ) ;
}
...
{	int		i ;
	...
	IrDA_memset( &tc->flags, 0, sizeof( TTPFLAGS )) ;
	tc->TxMaxSduSize	= 0 ;
	ttpInitCredit( &tc->AvailCredit, 0 ) ;
...
{	int		i ;
	...
	if( size > 0 && ttpGetCredit( &tc->RemoteCredit ) > 0 ) {
		...
		IrDA_memcpy( &tc->RxSduData[tc->RxSduWritePtr*tc->RxSduElmSize], data, size ) ;
...
static	int		ttpGetCredit( TTPCREDIT *tc )
{
	...
	for(;;) {
		...
		credit		= tc->plus.num + tc->plus.num2 -
						tc->minus.num - tc->minus.num2 ;
		if( credit == tc->plus.num + tc->plus.num2 -
					tc->minus.num - tc->minus.num2 )
			break ;
	}
