...
EXPORT_C VOID    MMIc_terminateUserAgent (UINT8 objectId)
{
    ...
    pMsg->Id = eMMIc_terminateUserAgent;
...
EXPORT_C VOID    MMIc_clearAuthenticationDatabase ( VOID )
{

#if     defined LOG_EXTERNAL
    CLNTa_log(0, 0, "MMIc_clearAuthenticationDatabase\nVOID\n");
#endif
...
*/
    ...
    pMsg->Id = eMMIc_textSigned;

    pMsg->vUINT8[0] = objectId;
    pMsg->vUINT8[1] = signId;
    pMsg->vUINT8[2] = algorithm;
    pCHAR = 0;
    if (sigLen)
    {
        if (signature)
    ...
    pMsg->vUINT16[2] = certificateLen;
    pMsg->pCHAR[2] = pCHAR;
    pMsg->vUINT8[3] = certificateType;
    pMsg->vUINT16[3] = err;
