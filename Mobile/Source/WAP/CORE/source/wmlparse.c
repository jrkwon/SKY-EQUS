					  ...
					  pELEMENTTYPE  pParent)
{
	if (*ppElement!=NULL)
	{
		...
		case Type_P:
			...
			if (WML_IsPEmptyInternal(pDecStr,*ppElement))
			{
				/* Remove p */
				XML_DeleteElement(ppElement,pDecStr);
			}
			else
			{
				/* Keep p and update current Wrap-mode */
				if (((pPELEMENT)(*ppElement))->iMode==P_Unknown)
				{
					((pPELEMENT)(*ppElement))->iMode=
		...
		case Type_ONEVENT:
			if (pParent!=NULL)
			{
				...
				if (pParent->iType==Type_OPTION)
				{
					...
					if ( ((pONEVENTELEMENT)(*ppElement))->iEventType==DEF_TYPE_ONCLICK)
					{
						/* Remove conflicting task - implies non-fatal error */
						XML_DeleteElement(&(((pOPTIONELEMENT)(pParent))->pOnpick),pDecStr);

						/* Move the Action to "attribute level" */
						((pOPTIONELEMENT)(pParent))->pOnpick=
								((pONEVENTELEMENT)(*ppElement))->pContent;

						/* Remove the pointer to the task to allow the
						   use of DeleteElement without deleting the task. */
						((pONEVENTELEMENT)(*ppElement))->pContent=NULL;
						break;
					}
				}
				...
				else if (((pParent->iType==Type_CARD)||(pParent->iType==Type_TEMPLATE))) 
				{
					...
					if ((((pONEVENTELEMENT)(*ppElement))->iEventType==DEF_TYPE_ONTIMER)||
						(((pONEVENTELEMENT)(*ppElement))->iEventType==DEF_TYPE_ONENTERFORWARD)||
						(((pONEVENTELEMENT)(*ppElement))->iEventType==DEF_TYPE_ONENTERBACKWARD))
					{
						/* The event is legal. Add event to the parent (CARD or TEMPLATE). */
						WML_AddToEventTable(pParent, (((pONEVENTELEMENT)(*ppElement))->iEventType),
								(((pONEVENTELEMENT)(*ppElement))->pContent));
						break;
					}
					...
					if (((UA*)(pDecStr->pAppSpec))->iUserAgentMode==User_Agent_WTA)
					{
						...
						*/
						if (((((pONEVENTELEMENT)(*ppElement))->iEventType>=WTAEvent_cc_ic)&&
							(((pONEVENTELEMENT)(*ppElement))->iEventType<=WTAEvent_ms_ns))

							/* GSM Specific */
							#ifdef CONFIG_WTAI_GSM

							/* Continue if-statement */
							|| ((((pONEVENTELEMENT)(*ppElement))->iEventType>=WTAEvent_gsm_ru)&&
							(((pONEVENTELEMENT)(*ppElement))->iEventType<=WTAEvent_gsm_ca))
							...
							/* Continue if-statement */
							|| ((((pONEVENTELEMENT)(*ppElement))->iEventType>=WTAEvent_is136_ia)&&
							(((pONEVENTELEMENT)(*ppElement))->iEventType<=WTAEvent_is136_if))
		...
		case Type_TEMPLATE:
			...
			((UA *)(pDecStr->pAppSpec))->pTemplateEvents =
						((pTEMPLATEELEMENT)(*ppElement))->pEventTab;
			break;
		}
	}
}
