...
wtls_log_client_hello (wtls_client_hello *ch)
{
  ...
  wtls_cvt_init (&cvt_obj, CVT_DECODE, ch->client_key_ids,
  ...
  while (cvt_obj.pos < cvt_obj.length) {
    ...
    wtls_log_msg (2, "]\n");
    wtls_cvt_init (&cvt_obj2, CVT_FREE, NULL, 0);
    wtls_cvt_key_exchange_id (&cvt_obj2, &kex);
  }
  wtls_log_msg (2, "\n");

  wtls_log_msg (2, "Trusted Key Ids:\n");
  wtls_cvt_init (&cvt_obj, CVT_DECODE, ch->trusted_key_ids,
...
wtls_log_handshake_msg (wtls_record_t *rec)
{
  ...
  wtls_cvt_init (&cvt_obj, CVT_DECODE_STATIC, rec->fragment, rec->length);
  ...
  case HANDSHK_CERTIFICATE:
#ifdef TBNG
    {
      ...
      wtls_cvt_init (&cvt_obj, CVT_DECODE_STATIC, ccert.buf, ccert.buflen);
      if (ccert.buflen > 0) {
