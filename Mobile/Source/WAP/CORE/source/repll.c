...
pCHCONTENTSTRUCT REP_GetStoredChannel (BYTE* pbData)
{
	...
	/* Copy expiry date */
	B_COPYSTRINGN(&(pChannel->iExpiryDate),pbData+3,4);
	iOffset+=7;
					   ...
					   UINT32* piContentID, UINT32* piSize)
{
	...
	if (pChannel!=NULL)
	{
		...
		iChLen=(15+iTitleLen*2+iAbstractLen*2+iEventIDLen*2+iChannelIDLen*2+iNbrRes*4);
									...
									->Storage, iChLen, &iContentID))
		{
										...
										iContentID, 0, iChLen, pbData))
			{
				...
				*piSize=iChLen;
				...
				/* Deduct from reserved memory */
				((REPCONTEXT*)pContext)->iReserved-=iChLen;
...
BOOL REP_AllocateMemory (void* pContext, UINT32 iSize)
{
	...
	/* Get memory left in repository */
	iFree=Storage_NumBytesFree(&((REPCONTEXT*)pContext)->Storage);
	
	/* Deduct the reserved memory */
	iFree-=((REPCONTEXT*)pContext)->iReserved;

	if (iSize<=iFree)
	{
		/* Reserve memory */
		((REPCONTEXT*)pContext)->iReserved+=iSize;
