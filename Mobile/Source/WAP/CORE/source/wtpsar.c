                      ...
                      TRInvokeReqType *req)
{
  ...
  tr = OSConnectorAlloc (sizeof (wtp_transaction_t));
  tr->tid = (UINT16)tid;
...
wtp_create_invoke_pdu (wtp_transaction_t *tr, wtp_invoke_segment_t *iseg)
{
  ...
  add_tpis = (iseg->segnum == 0) && !(iseg->flags & WTP_FLAG_TTR);
  length = (UINT16)(iseg->length + WTP_PDU_INVOKE_HEADERLEN);
  if (add_tpis) {
    /* This is the first segment in a segmented invoke.
     * We should add TPIs. */
    length += 8;
...
wtp_create_group (void *trp)
{
  ...
  if ((tr->flags & WTP_FLAG_MORE_DATA) ||
    ...
    while (length > 0) {
      iseg = OSConnectorAlloc (sizeof (wtp_invoke_segment_t));
      iseg->next = NULL;
  ...
  for (i = 4; i < n + 4; i++) {
    ...
    for (; iseg != NULL; iseg = iseg->next) {
      if (iseg->segnum == segnum) {
        mseg = OSConnectorAlloc (sizeof (wtp_missing_segment_t));
        mseg->iseg = iseg;
        mseg->next = NULL;
        if (q == NULL)
...
wtp_unpack_result_pdu (pdubuf *pdu)
{
  ...
  rseg = OSConnectorAlloc (sizeof (wtp_result_segment_t));
  rseg->pdu = pdu;
