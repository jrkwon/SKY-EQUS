	...
	if (pUser!=NULL)
	{
		...
		else
		{
			...
			if (pDecStr!=NULL)
			{
				...
				pDecStr->DTD_AddContent=WML_AddContent;
...
BOOL WML_ParseNextElement (pUA pUser) 
{
	...
	UINT16 iTag=0;
	BOOL fPiEnd=FALSE;
	BOOL fLiteralWML=FALSE;

	/* Check if user agent defined */
	if ((pUser==NULL)||(pUser->pDecStr==NULL))
	{
		return NULL;
	}
	
	/* Check if more data to decode */
	if ( pUser->pDecStr->pbCurByte < pUser->pDecStr->pbEnd )
	{
		if (pUser->pDeck==NULL)
		{
			/* wml-tag not found yet. Loop until end of byte stream 
			   or until an wml-element is found */
			while ( ( pUser->pDecStr->pbCurByte < pUser->pDecStr->pbEnd ) &&
					( pUser->pDecStr->iDecodeResult <= WBXML_FatalError) )
			{
				/* Get current byte token */
				bToken=*(pUser->pDecStr->pbCurByte);

				if ((bToken&MASK)==GLOBAL_LITERAL)
				{
					/* Get 'mask' */
					bTagInfo=(BYTE)(bToken&BOTH);					

					/* Step */
					pUser->pDecStr->pbCurByte++;

					/* Check if literal tag is wml */
					iRef=WBXML_GetMBUINT32(pUser->pDecStr);
					iTag=WML_LiteralTagToToken 
						(WBXML_GetStrTabVal (pUser->pDecStr, iRef));

					/* WTA Specific */
					#ifdef CONFIG_WTA
					if (pUser->iUserAgentMode==User_Agent_WTA)
					{
						WCHAR* pwchStr=WBXML_GetStrTabVal (pUser->pDecStr, iRef);

						if (pwchStr!=NULL)
						{
							if(COMPARESTRING (pwchStr,WTAWml)==0)
							{
								iTag=WTATAG_wtawml;
								fLiteralWML=TRUE;
								bToken=(BYTE)(WMLTAG_WML|bTagInfo);
							}
						}
					}
				...
				/* Get tag */
				iTag=(UINT16)(((bToken)&MASK) + (256 * pUser->pDecStr->iTagCodePage));

				if (((iTag==Type_WML)||(fLiteralWML))
					/* WTA Specific */
					#ifdef CONFIG_WTA
				...
				{
					/* Create wml-element */
					pContent=WML_CreateElement(pUser->pDecStr,Type_WML);
					...
					{
						/* Out of memory */
						pUser->pDecStr->iDecodeResult|=WBXML_Error_OutOfMemory;
						return FALSE;
					}
					#endif

					/* Store element */
					pUser->pDeck=pContent;

					/* Decode attributes (if any) */
					if (bToken&ATTR)
					{
						WBXML_DecodeAttributes (pUser->pDecStr,pContent);
					}

					/* Decode content - if any (return TRUE) */
					if (bToken&CONT)
					{
						return TRUE;
					}
				...
				else if (bToken==GLOBAL_PI)
				{
					...
					while ( (pUser->pDecStr->pbCurByte < pUser->pDecStr->pbEnd) && 
							(!fPiEnd) )
					{
						if ( *(pUser->pDecStr->pbCurByte)==GLOBAL_END )
						{
							/* END found - Step past and return TRUE to 
							   continue decoding. */
							pUser->pDecStr->pbCurByte++;
							fPiEnd=TRUE;
						}
				...
				else if (bToken==GLOBAL_SWITCH_PAGE)
				{
					pUser->pDecStr->pbCurByte++;
					...
					if ( pUser->pDecStr->pbCurByte < pUser->pDecStr->pbEnd )
					{
						...
						pUser->pDecStr->iTagCodePage=*pUser->pDecStr->pbCurByte;
						pUser->pDecStr->pbCurByte++;
					}
				}
				else
				{
					/* Error - MUST be wml or pi */
					pUser->pDecStr->iDecodeResult|=WBXML_Error_Encoding;
				}
			}
		}
		else
		{
			/* Loop until end of byte stream or until an element is found */
			while (pUser->pDecStr->pbCurByte < pUser->pDecStr->pbEnd)
			{
				/* Get current byte token */
				bToken=*(pUser->pDecStr->pbCurByte);
				...
				case GLOBAL_EXT_2:
					/* Decode and discard */
					pContent=WBXML_DecodeGlobalText(pUser->pDecStr);
					XML_DeleteElement(&pContent,pUser->pDecStr);
					pUser->pDecStr->iDecodeResult|=WBXML_Warning_IllegalElement;
					break;
				...
				case GLOBAL_OPAQUE:
					...
					/* Get length (mbuint32) */
					iLength=WBXML_GetMBUINT32 (pUser->pDecStr);

					/* Step past data */
					pUser->pDecStr->pbCurByte+=iLength;
					pUser->pDecStr->iDecodeResult|=WBXML_Warning_IllegalElement;
					break;
				...
				/* END - end of wml deck, return FALSE */
				case GLOBAL_END:
					pUser->pDecStr->pbCurByte++;
					return FALSE;

				/* CARD, HEAD or TEMPLATE, SWITCH_PAGE, or LITERAL. If found, 
				   the function WBXML_DecodeElement is called.
				*/
				default:
					/* Parent is wml (Deck in User Agent) */

					/* Set default wrap mode (Wrap) */
					pUser->iCurrentWrapMode=P_Wrap;

					pContent=WBXML_DecodeElement(pUser->pDecStr,pUser->pDeck);

					/* Add content to deck if not fatal error */
					if ( ( pUser->pDecStr->iDecodeResult <= WBXML_FatalError) &&
						 ( WML_AddContent (pUser->pDecStr,pUser->pDeck,&pContent)))
					{
						/* Continue decoding */
						return TRUE;
					}
					...
					{
						/* Could not store content - delete */
						XML_DeleteElement(&pContent,pUser->pDecStr);
						pUser->pDecStr->iDecodeResult|=WBXML_Warning_IllegalElement;
						return FALSE;
					}
	...
	{
		/* END-tag for wml not found. Error in encoding */
		pUser->pDecStr->iDecodeResult|=WBXML_Error_Encoding;
	}
	return FALSE;
}
	...
	*pwchString=0;
	...
	while ((pTemp!=NULL)&&(fText))
	{
		...
		else if (pTemp->iType==Type_Ext_0)
		{
			...
			/* URL-escape text. */
			pwchExtEscaped=w_EscapeString(pwchExt);
...
UINT32 WML_GetStringLength (pUA pUser, pELEMENTTYPE pElement)
{
	...
	while ((pTemp!=NULL)&&(fText))
	{
		...
		else if (pTemp->iType==Type_Ext_0) 
		{
			...
			/* URL-escape text. */
			pwchEscText=w_EscapeString(pwchTempText);
			if (pwchEscText!=NULL)
			{
