...
#ifdef WTLS_STATUS_CALL
  if (!(conn->h_state->msgs & BIT_SERVER_HELLO_DONE)) {
    status_code = WTLS_STATUS_ABBREVIATED_HANDSHAKE;
  }
  else if (conn->h_state->msgs & BIT_CLIENT_CERTIFICATE) {
    status_code = WTLS_STATUS_FULL_HANDSHAKE_CLIENT_AUTH;
  }
  else if (conn->h_state->msgs & BIT_SERVER_CERTIFICATE) {
    status_code = WTLS_STATUS_FULL_HANDSHAKE_SERVER_AUTH;
  }
  else {
    status_code = WTLS_STATUS_FULL_HANDSHAKE_ANON;
  }

  CRYPTa_status (GENERAL_VIEW, status_code,
                 conn->h_state->key_exch.key_exchange_suite,
                 conn->write.cobj.mac_alg,
                 conn->write.cobj.bulk_cipher_alg,
                 sh->session_id.id, sh->session_id.length,
                 addr, addr[1] + 2,
                 conn->server_addr.PortNumber);
#endif

  MMIa_status (conn->server_addr.ViewID, WTLSConnectionEstablished, 0);
                        ...
                        SDL_Natural creation_time)
{
  ...
  if ((result == CRV_OK) &&
    ...
    return RET_USING_CURRENT_SESSION;
  }
...
wtls_cm_process_server_hello (void *connptr, void *bufptr, SDL_Natural buflen)
{
  ...
  wtls_cvt_init (&cvt_obj, CVT_DECODE_STATIC, buf, (UINT16)buflen);
...
wtls_cm_create_finished (void *connptr, void **pbufptr, SDL_Natural *pbuflen)
{
  ...
  wtls_cvt_init (&cvt_obj, CVT_ENCODE, buf, 15);
...
wtls_cm_terminate (void *connptr, SDL_Boolean established)
{
  ...
  if (established == SDL_True) {
    MMIa_status (conn->server_addr.ViewID, WTLSConnectionTerminated, 0);
                                   ...
                                   wtls_server_hello *sh)
{
  ...
  wtls_cvt_init (&cvt_obj, CVT_DECODE_STATIC, ch->client_key_ids,
