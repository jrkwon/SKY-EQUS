...
#define MAX_DATA 536
#define UDPERROR
#define WAP_USE_CHECK_DSMBUFFER
...
BOOL bCall_content = FALSE;
	...
	#include "..\WAP_B1\secure\cwsplib.h"
	#include "..\WAP_B1\secure\const.h"
	#include "..\WAP_B1\secure\cssls.h"
  ...
  while(payload_len!=0)
  {
	if(fstaccess)
	{
...
#ifdef WAP_USE_CHECK_DSMBUFFER
			if(payload_ptr == NULL) 
			{                
                resetalldata();
				UDPc_errorRequest(2,sourcePort);
                CLNTa_log(1,0,"WAP has no DSM1 items!");
				return;
			}
#endif//#ifdef WAP_USE_CHECK_DSMBUFFER		        
		...
		else
		{
			...
			{                
                resetalldata();
				UDPc_errorRequest(2,sourcePort);
                CLNTa_log(1,0,"WAP has no DSM2 items!");
				return;
            }
#endif//#ifdef WAP_USE_CHECK_DSMBUFFER
		}
	...
	else
	{
        if((payload_len+sizeof(udp_hdr_type)+4) < DSM_DS_SMALL_ITEM_SIZ)
		{
			...
			{
		        //Error처리 할것
				CLNTa_log(1,0,"WAP has no DSM3 items!");
#ifdef WAP_USE_CHECK_DSMBUFFER
				if(payload_ptr != NULL) 
				{
		...
		else
		{
			...
			{
		        //Error처리 할것
				CLNTa_log(1,0,"WAP has no DSM4 items!");
#ifdef WAP_USE_CHECK_DSMBUFFER
				if(payload_ptr != NULL) 
				{
					dsm_free_packet(&payload_ptr);
	...
	if(payload_len < 0 )
	{
        CLNTa_log(1,0,"WAP has no DSM5 items!");
#ifdef WAP_USE_CHECK_DSMBUFFER
        if(payload_ptr !=NULL)
        {
	        dsm_free_packet(&payload_ptr);
