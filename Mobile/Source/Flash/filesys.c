...
extern nv_fn_sequence_type Fn_sequence[NV_MAX_FL_FILE_NUM]; // defined in VocCommon.c
#else
nv_fn_sequence_type Fn_sequence[NV_MAX_FL_FILE_NUM]; // defined in VocCommon.c
#endif
...
void erase_unused_Fn(void)
{
	...
	for(count=0;count<NV_MAX_FL_FILE_NUM;count++)
	{
		...
		if( (ui.voice_info.is_user_voice == TRUE) && 
			(count ==ui.voice_info.user_voice_fn))
		...
		else
		{
			...
			for(n=0;n<ui.voice_info.total_rec_num;n++)
			{
				if(Fn_sequence[n].Fn == count)
					break;
			}
			if(n==ui.voice_info.total_rec_num)
			{
				flferase(count);
			}
		}
...
//---------------------------------------------------------------------------------------------------
// voice_info
	if (NV_DONE_S != ui_get_nv(NV_VOICE_INFO_I, (nv_item_type *)&voice_info))
		return FALSE;
...
// Fn_sequence
	for (i = 0; i < NV_MAX_FL_FILE_NUM; i++)
	{
		...
		if (NV_DONE_S != ui_get_nv(NV_FN_SEQUENCE_I, (nv_item_type *)&Fn_sequence[i]))
			return FALSE;
	}
//---------------------------------------------------------------------------------------------------
	erase_unused_Fn();
...
boolean vd_read_superblock(void)
{
	...
	if (NV_DONE_S != ui_get_nv(NV_VOICE_TEMPLETE_HEADER_I, (nv_item_type *)&vDial_tHeader))
		return FALSE;
...
#if defined(FEATURE_EMAIL) || defined(FEATURE_NETWORK)
/////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////
// For E-mail jmpark 000313

#define DEVOFFSET   29
#define HEAPOFFSET  30
#define PROXYOFFSET 31
#define NVOFFSET    32

#define NV_MAX_DATA_FILE_NUM (NVOFFSET+1) 
...
//save for DevPermWrite
   {"data29", FS_FA_UNRESTRICTED, FS_NULL_HANDLE, FS_NULL_HANDLE,0},
//save for HeapMemory
   {"data30", FS_FA_UNRESTRICTED, FS_NULL_HANDLE, FS_NULL_HANDLE,0},
//save for ProxyTable
   {"data31", FS_FA_UNRESTRICTED, FS_NULL_HANDLE, FS_NULL_HANDLE,0},
//save for NVTable
   {"data32", FS_FA_UNRESTRICTED, FS_NULL_HANDLE, FS_NULL_HANDLE,0},
};
...
boolean File_DevPermRead( char* data_buf, int length )
{
	boolean result = FALSE;
	byte option_type = 0; //read

	memset((char*)data_buf, NULL, length);
	result = data_flfopen(DEVOFFSET, option_type);
	if(result)
	{
		data_flfread(DEVOFFSET, (byte*)data_buf, length );
	}
	data_flfclose(DEVOFFSET);
	return TRUE;
}

boolean File_DevPermWrite( char* data_buf, int length )
{
	boolean result = FALSE;
	byte option_type = 1; //write

	data_flferase(DEVOFFSET);
	result = data_flfopen(DEVOFFSET, option_type);
	if(result)
	{
		data_flfwrite(DEVOFFSET, (byte*)data_buf, length );
	}
	data_flfclose(DEVOFFSET);
	return result;
}

boolean File_HeapRead( char* data_buf, int length )
{
	boolean result = FALSE;
	byte option_type = 0; //read

	memset((char*)data_buf, NULL, length);
	result = data_flfopen(HEAPOFFSET, option_type);
	if(result)
	{
		data_flfread(HEAPOFFSET, (byte*)data_buf, length );
	}
	data_flfclose(HEAPOFFSET);
	return TRUE;
}

boolean File_HeapWrite( char* data_buf, int length )
{
	boolean result = FALSE;
	byte option_type = 1; //write

	data_flferase(HEAPOFFSET);
	result = data_flfopen(HEAPOFFSET, option_type);
	if(result)
	{
		data_flfwrite(HEAPOFFSET, (byte*)data_buf, length );
	}
	data_flfclose(HEAPOFFSET);
	return result;
}

boolean File_ProxyTable( char* data_buf, int length, int readwrite )
{
	boolean result = FALSE;
	byte option_type = readwrite; //read 0, write 1

	if(option_type == 0)
	{
		memset((char*)data_buf, NULL, length);
		result = data_flfopen(PROXYOFFSET, option_type);
		if(result)
		{
			data_flfread(PROXYOFFSET, (byte*)data_buf, length );
		}
		data_flfclose(PROXYOFFSET);
		return TRUE;
	}else if(option_type == 1)
	{
		data_flferase(PROXYOFFSET);
		result = data_flfopen(PROXYOFFSET, option_type);
		if(result)
		{
			data_flfwrite(PROXYOFFSET, (byte*)data_buf, length );
		}
		data_flfclose(PROXYOFFSET);
		return result;
	}
...
boolean File_NVTable( char* data_buf, int length, int readwrite )
{
	boolean result = FALSE;
	byte option_type = readwrite; //read 0, write 1
	...
	{
		memset((char*)data_buf, NULL, length);
		result = data_flfopen(NVOFFSET, option_type);
		if(result)
		{
			data_flfread(NVOFFSET, (byte*)data_buf, length );
		}
		data_flfclose(NVOFFSET);
		return TRUE;
	}else if(option_type == 1)
	{
		data_flferase(NVOFFSET);
		result = data_flfopen(NVOFFSET, option_type);
		if(result)
		{
			data_flfwrite(NVOFFSET, (byte*)data_buf, length );
		}
		data_flfclose(NVOFFSET);
		return result;
	}
...
#endif //FEATURE_EMAIL || FEATURE_NETWORK
// For E-mail End
