...
wip_initmalloc (void *mem, UINT32 memsize)
{
  ...
  {
	ERR_FATAL("wip_initmalloc() fail",0,0,0);
    return -1;
  }
  baseptr = mem;
...
//#ifdef DEBUG_WIP_MALLOC
      tmp_traverse_count++;
	  if(tmp_traverse_count > 500)
	  {
		  ERR_FATAL("exceed time to find out a memory",0,0,0);
	  }
//#endif
 ...
 found:
  ...
  memset( ((void*)((unsigned char *)(p) + CHUNKHEADERSIZE)), 0, size);
  return chunk2mem (p);
...
//#ifdef DEBUG_WIP_MALLOC
	...
	if(tmp_traverse_count > 500)
	{
	  ERR_FATAL("exceed time to clear memory",0,0,0);
	}
//#endif
  }
}
