                            ...
                            SDL_Natural public_valuelen)
{
  ...
  CRYPTa_sessionInvalidate (master_secret_id);
                                      ...
                                      BYTE **id_buf, UINT16 *idlen)
{
  ...
  *idlen = (UINT16)cvt_obj.pos;
  *id_buf = NEWARRAY (BYTE, *idlen);
  if (*id_buf == NULL) {
    return -1;
  }
  wap_cvt_init (&cvt_obj, WAP_CVT_ENCODE, *id_buf, *idlen);
  if (!wtls_cvt_identifier (&cvt_obj, &(conn->h_state->key_exch.identifier))) {
    wtls_err_set (ERR_INTERNAL, ERR_ENCODING, 1,
                  ALERT_LEVEL_CRITICAL, ALERT_DESC_INTERNAL_ERROR);
    return -1;
