...
==========================================================================
    ...
    010330      ASPN    Function LL_SizeOfStoredRes removed
						...
						UINT32 iLLRepHIdToFirstRes)
{
	...
	if( pvRepChList != NULL ){
		...
		if( pRepChStruct != NULL ){
            ...
            pRepChStruct->bServiceAccessible = TRUE;
...
void ME_ServiceList_Add(void** ppvServiceList, void* pvRepContent)
{
	...
	if (ppServiceList != NULL) {
		...
		pServiceTypeStruct->wtaChannelId = pChContent->pwchchannelid;
		pServiceTypeStruct->useraccess = pChContent->useraccess;
        pServiceTypeStruct->bServiceAccessible = pChContent->bServiceAccessible;
		...
		*ppServiceList = NULL;
        ...
        DEALLOC(&(pChContent->pwchEventId));
        DEALLOC(&(pChContent) );
...
void ME_ServiceList_Delete(void** ppvServiceList)
{
	...
	if (ppServiceList != NULL) {
		...
		while (*ppServiceList != NULL) {
			...
			DEALLOC( &(pServiceTypeStruct->wtaChannelId) );
								 ...
								 void* pvChannelType )
{
    ...
    pChContent->bServiceAccessible = TRUE;
