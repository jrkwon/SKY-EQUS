...
enumErrorCode
Call_Crypto_signText (pstructSI si, UINT16 libIndex, UINT8 funcIndex)
{
  ...
  do {
    ...
    si->sc = ST_newSignedContent ();
    if (si->sc == NULL)
      break;

    utf8StrLen = Unicode2UTF8_calcLen (stringToSign->val.theString,
                                       stringToSign->theStringLen);
    utf8Str = NEWARRAY (BYTE, utf8StrLen);
    if (!Unicode2UTF8_convert (stringToSign->val.theString,
                               stringToSign->theStringLen,
                               utf8Str, utf8StrLen, &dummy))
      break;

    pchKeyId = NEWARRAY (CHAR, keyId->theStringLen);
    ...
    if (0 != ST_createDigestInfo (utf8Str, utf8StrLen,
                                  digestinfo, &diglen,
                                  (UINT8)(options->val.theInt), si->sc))
      break;

    MMIa_signText (si->theUserAgent->iViewID, si->invokeID,
                   stringToSign->val.theString, digestinfo, diglen, 
                   (UINT8)(keyIdType->val.theInt), pchKeyId,
                   (UINT16)(keyId->theStringLen),
                   (UINT8)(options->val.theInt & ~1));
...
enumErrorCode Lib_ProprietaryLibraryFunctionResponse( pstructSI si, WMLSvar **pResultVar )
{
	...
	if (*pResultVar)
	{
		...
		WMLSvar_Delete( pResultVar, TRUE );
	}
