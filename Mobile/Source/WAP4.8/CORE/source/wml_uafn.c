...
int SDL_CheckResponseContentType(const void* pResponseInfo, const void* pCompareContentType)
{
    ...
    if (pResponseInfo==NULL)  return FALSE;
    
	pContentTypeText = ((const CONTENTINFOSTRUCT*)pResponseInfo)->pContentTypeAsText;

#ifdef WAP_USE_CHARACTERBELL
	Contentint = ((const CONTENTINFOSTRUCT*)pResponseInfo)->iContentTypeAsInt;
#endif//#ifdef WAP_USE_CHARACTERBELL
	...
	if(Contentint == 0x6A)
	{
		pbAcceptContentType = NEWARRAY(BYTE,B_STRINGLENGTH((const char *)ACCEPT_CHACTERBELL)+1);
	}
	else
#endif//#ifdef WAP_USE_CHARACTERBELL
	...
	if(Contentint == 0x6A)
	{
		if (pbAcceptContentType)
			DowncaseString (pbAcceptContentType, (BYTE*)ACCEPT_CHACTERBELL);
	}
	else
#endif//#ifdef WAP_USE_CHARACTERBELL
	{
...
*/
   ...
   MSG_ERROR("SDL_GetElementURL START",0,0,0);
   if (pURLElement!=NULL)
   {
     ...
     switch (SDL_GetElementType(pURLElement))
	 {
		 ...
		 case Type_GO:
			 ...
			 MSG_ERROR("evalute the postfields etc.",0,0,0);
   ...
   *pURL = pbResult;
   ...
   CLNTa_log(1,0,"REL  URL [%s]",pbTemp);
	...
	memset(gWAPRedirectUrl,0x00,1500);
	if(strlen((const char *)pbResult) < 1500 ) 
     strncpy(gWAPRedirectUrl,(const char *)pbResult,strlen((const char *)pbResult));
    ...
    if ( pWTAIFunction && pWTAISubFunction )
    {
      ...
      if (pWTAINumber)
      {
        ...
        if (pWTAIFrag != NULL)
        {
          ...
          pTmpArray[len] = '#';   /* include '#' */
          B_COPYSTRING((char *)&pTmpArray[len+1], (const char *)pWTAIFrag);
          DEALLOC(& pWTAINumber);
          pWTAINumber = pTmpArray;
        }
       
        DEALLOC(& pWTAIFrag);
      
        /* Check if public URI function */
        if (strcmp("wp",(const char *)pWTAIFunction)==0)
        {
          /* Remove the <result> section */ /*TODO:handle <result> instead of discarding*/
          BYTE *p;
          for (p = pWTAINumber; *p; p++) {
            BYTE b = *p;
            if ((b == '!')) {
              UINT16 len = (UINT16)(p - pWTAINumber);
              BYTE* pTmpArray;
              pTmpArray = NEWARRAY (BYTE, len+1);
              strncpy((char *)pTmpArray, (const char *)pWTAINumber, len);
              pTmpArray[len] = '\0';
              DEALLOC(& pWTAINumber);
              pWTAINumber = pTmpArray;
              break;
      ...
      /* Check if endContext function */
      else if ((strcmp("ms",(const char *)pWTAIFunction)==0) && 
               (strcmp("/ec",(const char *)pWTAISubFunction)==0)   )
      {
        iRet = WTAI_URI_END_CONTEXT;
      }
    } /* No function and/or sub function */
