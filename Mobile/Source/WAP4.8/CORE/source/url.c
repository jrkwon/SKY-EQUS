...
#ifdef FEATURE_WAP
    p = (BYTE *)strchr ((char *)url.s[PATH_PART], '/');
#else
...
b_AppendToQuery (BYTE *old_url, BYTE *new_query)
{
  ...
  if ((new_query == NULL) || ((nqlen = B_STRINGLENGTH (new_query)) == 0))
  ...
  if (url.s[QUERY_PART] == NULL) {
    ...
    B_COPYSTRING (tmp, url.s[QUERY_PART]);
    tmp[url.len[QUERY_PART]] = '&';
    B_COPYSTRING (tmp + url.len[QUERY_PART] + 1, new_query);
...
b_LongestCommonPrefix (BYTE *s1, BYTE *s2)
{
  ...
  len1 = B_STRINGLENGTH (s1);
  len2 = B_STRINGLENGTH (s2);
  if ((len1 == 0) || (len2 == 0))
    return NULL;
  if ((s1[0] != '/') || (s2[0] != '/'))
    return NULL;
