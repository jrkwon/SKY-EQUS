...
typedef struct
{
	...
	char				chWaitingFor;		/* Kan ev strykas om man går på SID */
...
int GetCacheControl( pHEADERELEMENT pHeadList )
{
	...
	if( pHeadList != NULL ){
		...
		if( pHeaderElement == NULL ){
			pHeaderElement = WSP_GetHeaderWK ( 1, Field_Cache_Control_1_3, pHeadList );
			if( pHeaderElement == NULL )
				pHeaderElement = WSP_GetHeaderWK ( 1, Field_Cache_Control_1_4, pHeadList );
		}
		if( pHeaderElement != NULL )
		{
			  ...
			  int iProxyCredentialsLength, VOID** pvHeader )
{
	...
	UINT8		uiLen = 0;
	BYTE		tempArray[2];
	BYTE		tempArray2[38]={'E','n','c','o','d','i','n','g','-','V','e','r','s','i','o','n',0x00,'E','n','c','o','d','i','n','g','-','V','e','r','s','i','o','n',0x00,'1','.','3',0x00};
	DB_ref		globlDB_Ref = 0;
	UINT8		uiErrorCode = 0;
		...
		if( iConnectionMode == 0 && NR_OF_ACCEPTCHARSET > 0 ){
			for ( i=0; i<NR_OF_ACCEPTCHARSET; i++ ){
				if( arrayAcceptCharset[i] < 256 ){
					...
					if( iEncodingVersion == ENCODING_VERSION_1_2 || iEncodingVersion == 0 )
						WSP_CreateHeaderField( Field_Accept_Charset, tempArray, 1, &pList);
					else
						WSP_CreateHeaderField( Field_Accept_Charset_1_3, tempArray, 1, &pList);
				}
				else{
					...
					if( iEncodingVersion == ENCODING_VERSION_1_2 || iEncodingVersion == 0 )
						...
						WSP_CreateHeaderField( Field_Accept_Charset_1_3, tempArray, 2, &pList);
			...
			if( pWAEMainStruct != NULL ){
				if( pWAEMainStruct->pbPROFILE_DIFF != NULL && pWAEMainStruct->iPROFILE_DIFF_LEN > 0 &&
					!(*(pWAEMainStruct->pbPROFILE_DIFF) == 0 && pWAEMainStruct->iPROFILE_DIFF_LEN == 1) ){
							/* This header would also be a part of the Connection header, because of this header is a hop by hop header.*/
...
/* Only in ConnectionLess Mode */
		if( iConnectionMode == 0){
			if( iEncodingVersion == 0 || iEncodingVersion == ENCODING_VERSION_1_3 )
				tempArray2[36] = '3';
			else if( iEncodingVersion == ENCODING_VERSION_1_2 )
				tempArray2[36]= '2';
			else
				tempArray2[36]= '4';
			WSP_CreateHeaderField( Field_Connection, tempArray2, 38, &pList);
			if( pStartList == NULL )
				pStartList = pList;
		}
#ifdef FEATURE_WAP
      globlDB_Ref = db_getRef( DB_root, (const BYTE *)"globl", &uiErrorCode );
#else
      globlDB_Ref = db_getRef( DB_root, "globl", &uiErrorCode );
#endif //#ifdef FEATURE_WAP

      if( !uiErrorCode )
#ifdef FEATURE_WAP
         i = (UINT16) db_getInt( globlDB_Ref, (const BYTE *)COOKIES_ENABLED, FALSE, &uiErrorCode );
#else
         i = (UINT16) db_getInt( globlDB_Ref, COOKIES_ENABLED, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP

		if( !uiErrorCode && i && iEncodingVersion != 0 && iEncodingVersion != ENCODING_VERSION_1_2 && checkCookies( pUrlReq->pbURL ) ){
			pbString = NULL;
			uiLength = createCookieData( &pbString );
			if( uiLength > 0 && pbString != NULL ){
				WSP_CreateHeaderField( Field_Cookie, pbString, uiLength, &pList);
				if( pStartList == NULL )
					pStartList = pList;
			}
			DEALLOC(&pbString);
	...
	if( uiLength > 0 ){
		...
		if( pbOctetString != NULL ){
			...
			if( iReload ){
				if( uiIndex == 0)
					uiIndex = uiStart;
				if( iEncodingVersion == 0 || iEncodingVersion == ENCODING_VERSION_1_2 )
					pbOctetString[uiIndex] = 0x80 + Field_Cache_Control;
				else if( iEncodingVersion == ENCODING_VERSION_1_3 )
					pbOctetString[uiIndex] = 0x80 + Field_Cache_Control_1_3;
				else
					pbOctetString[uiIndex] = 0x80 + Field_Cache_Control_1_4;
				uiIndex++;
					/* No-Cache */
...
void manageCookies( void* pHeaderHandle, BYTE* pbURL )
{
	...
	UINT32		uiExpires = 0;
	UINT32		uiVersion = 0;
	UINT32*		puiExpires = NULL;

	if( pHeaderHandle != NULL )
	{
		pHeaderList = (pHEADERDEF)pHeaderHandle;
		if( pHeaderList->pHeadList != NULL ){		/* Several CookieHeaders ?? */
			pHeaderElement = WSP_GetHeaderWK ( 1, Field_Set_Cookie, pHeaderList->pHeadList );
			if( pHeaderElement != NULL ){
				do {
					puiExpires = &uiExpires;
					WSP_GetSetCookie( pHeaderElement, &fError, &pbName,		
									&pbValue, &pbDomain, &pbPath, &puiExpires, &uiVersion );
						/* *puiExpires (if any) is here max age, i.e. seconds relative now */
					if( !fError ){
						/* if *puiExpires exist and is not 0 it must be given as seconds from 1970 */
						if (puiExpires && (*puiExpires > 0))
							*puiExpires += CLNTa_currentTime( );
						storeCookie( pbURL, pbName, pbValue, pbDomain, pbPath, puiExpires, uiVersion );
					}
					pHeaderElement = WSP_GetHeaderWK ( 1, Field_Set_Cookie, pHeaderElement->pNext );
				} while( pHeaderElement != NULL );
			}
		}
	}
...
Comment: Cookie-value = Value-length Cookie-version *Cookie
         Value-length = <octet 0-30> | (<octet 31> Uintvar-integer>)
         Cookie = Cookie-length Cookie-name Cookie-val [Cookie-parameters]
         Cookie-length = Uintvar-integer
         Cookie-parameters = Path [Domain]
         Path = Text-string
         ;an empty path string is allowed
         Domain = Text-string
=========================================================================*/
UINT32 createCookieData( BYTE** ppbString )
{
   ...
   *ppbString = NULL;
   ...
   if (!getNextCookie(&pbName, &pbValue, &pbDomain, &pbPath, &uiVersion))
      return 0;

   /* Only matched cookies with the same version as the first cookie */
   /* are encoded. */
   uiSharedVers = uiVersion;

   /* Calculate resulting string size: uiLength. */
   /* Bytes needed for Value-length is calculated last. */
   uiLength = 1; /* byte for version which always is short integer here */
   do
   {
      if (uiVersion != uiSharedVers)
         continue;

      /* For each text-string, the bytes needed in WAP are the string  */
      /* length, the byte for the terminating 0 char and if the first  */
      /* character >= 128 a byte is needed for a qoute prefix char. */ 
      uiTmpLen  = B_STRINGLENGTH(pbName);
      uiTmpLen += isWapCharCh(pbName[0]) ? 1 : 2;
      uiTmpLen += B_STRINGLENGTH(pbValue);
      uiTmpLen += isWapCharCh(pbValue[0]) ? 1 : 2;
      if (pbPath)
      {
         uiTmpLen += B_STRINGLENGTH(pbPath);
         uiTmpLen += isWapCharCh(pbPath[0]) ? 1 : 2;
      }
      else
         uiTmpLen += 1;
      uiTmpLen += B_STRINGLENGTH(pbDomain);
      uiTmpLen += isWapCharCh(pbDomain[0]) ? 1 : 2;

      uiLength += UintvarLen(uiTmpLen);
      uiLength += uiTmpLen;
   } while (getNextCookie(&pbName, &pbValue, &pbDomain, &pbPath,
                          &uiVersion));
   /* calculate bytes needed for Value-length */
   uiValLen = uiLength;
   if (uiValLen <= 30)
      uiTmpLen = 1;
   else
      uiTmpLen = UintvarLen(uiValLen) + 1;
   uiLength += uiTmpLen;

   *ppbString = (BYTE*) OSConnectorAlloc(uiLength);
   #ifndef HAS_SETJMP
   if (*ppbString == NULL) {
      return 0;
   }
   ...
   {
      *pbTmp++ = (BYTE)(31);
      pbTmp += IntToUintvar(uiValLen, pbTmp);
   }
   else
      *pbTmp++ = (BYTE)(uiValLen);

   *pbTmp++ = (BYTE) ((uiSharedVers << 4) | 0x8F);
                        ...
                        &uiVersion))
   {
      ...
      uiNameLen = B_STRINGLENGTH(pbName);
      uiTmpLen = uiNameLen;
      uiTmpLen += isWapCharCh(pbName[0]) ? 1 : 2;
      uiValLen = B_STRINGLENGTH(pbValue);
      uiTmpLen += uiValLen;
      uiTmpLen += isWapCharCh(pbValue[0]) ? 1 : 2;
      if (pbPath)
      {
         uiPathLen = B_STRINGLENGTH(pbPath);
         uiTmpLen += uiPathLen;
         uiTmpLen += isWapCharCh(pbPath[0]) ? 1 : 2;
      }
      else
         uiTmpLen += 1;
      uiDomLen = B_STRINGLENGTH(pbDomain);
      uiTmpLen += uiDomLen;
      uiTmpLen += isWapCharCh(pbDomain[0]) ? 1 : 2;

		pbTmp += IntToUintvar(uiTmpLen, pbTmp);

      if (!isWapCharCh(pbName[0]))
         *pbTmp++ = 0x7F;
      B_COPYSTRING(pbTmp, pbName);
      pbTmp += uiNameLen + 1;

      if (!isWapCharCh(pbValue[0]))
         *pbTmp++ = 0x7F;
      B_COPYSTRING(pbTmp, pbValue);
      pbTmp += uiValLen + 1;

      if (pbPath)
      {
         if (!isWapCharCh(pbPath[0]))
            *pbTmp++ = 0x7F;
         B_COPYSTRING(pbTmp, pbPath);
         pbTmp += uiPathLen + 1;
      }
      else
         *pbTmp++ = 0;

      if (!isWapCharCh(pbDomain[0]))
         *pbTmp++ = 0x7F;
      B_COPYSTRING(pbTmp, pbDomain);
      pbTmp += uiDomLen + 1;
...
void*  getMyPtrFromMem( DB_ref record, const BYTE* key, BOOL del, UINT8* errorCode, UINT16* uiLength )
{
	...
	*uiLength = 0;
	bDummy = (BYTE*)db_getPtr( record, key, del, errorCode );
	if( bDummy != NULL ){
...
int setNetworkParameters( VOID* pvWAEMaStruct, VOID* pvNetwork, VOID* pvUaSettings, int iObjectID, int iChannelRef, int iConnectionOriented )
{
	...
	if( pvNetwork != NULL ){
...
#ifdef FEATURE_WAP
		iDummy = (int)db_getInt( dbChannelRef, (const BYTE*)STACKMODE, FALSE, &uiErrorCode );
#else
        iDummy = (int)db_getInt( dbChannelRef, STACKMODE, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		iDummy = (int)db_getInt( dbChannelRef, (const BYTE*)ACCESS_TYPE, FALSE, &uiErrorCode );
#else
        iDummy = (int)db_getInt( dbChannelRef, ACCESS_TYPE, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		iDummy = (int)db_getInt( dbChannelRef, (const BYTE*)CLIENT_LOCAL_PORT, FALSE, &uiErrorCode );
#else
        iDummy = (int)db_getInt( dbChannelRef, CLIENT_LOCAL_PORT, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		vDummy = getMyPtrFromMem( dbChannelRef, (const BYTE*)UDP_IP_SRC, FALSE, &uiErrorCode, &iDummy );
#else
        vDummy = getMyPtrFromMem( dbChannelRef, UDP_IP_SRC, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
				(strncmp((CHAR*) vDummy, (char *)pNetwork->pbUDP_CLIENT_IP, iDummy) !=0) ) ){
#else
                (strncmp((CHAR*) vDummy, pNetwork->pbUDP_CLIENT_IP, iDummy) !=0) ) ){
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		vDummy = getMyPtrFromMem( dbChannelRef, (const BYTE *)UDP_IP_GW, FALSE, &uiErrorCode, &iDummy );
#else
        vDummy = getMyPtrFromMem( dbChannelRef, UDP_IP_GW, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
		if( vDummy != NULL && 
				(pNetwork->pbUDP_GW_IP==NULL || 
				(iDummy!=pNetwork->pbUDP_GW_IP_LEN) ||
#ifdef FEATURE_WAP
				(strncmp((CHAR*) vDummy,(char *)pNetwork->pbUDP_GW_IP,iDummy)!=0) ) ){
#else
                (strncmp((CHAR*) vDummy,pNetwork->pbUDP_GW_IP,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		vDummy = getMyPtrFromMem( dbChannelRef, (const BYTE *)SMS_C, FALSE, &uiErrorCode, &iDummy );
#else
        vDummy = getMyPtrFromMem( dbChannelRef, SMS_C, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
				(strncmp((CHAR*) vDummy,(char *)pNetwork->pbSMS_CENTER,iDummy)!=0) ) ){
#else
                (strncmp((CHAR*) vDummy,pNetwork->pbSMS_CENTER,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		vDummy = getMyPtrFromMem( dbChannelRef, (const BYTE *)SMS_GW, FALSE, &uiErrorCode, &iDummy );
#else
        vDummy = getMyPtrFromMem( dbChannelRef, SMS_GW, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
		if( vDummy != NULL && 
				(pNetwork->pbSMS_GW==NULL || 
				(iDummy!=pNetwork->pbSMS_GW_LEN) ||
#ifdef FEATURE_WAP
				(strncmp((CHAR*) vDummy,(char *)pNetwork->pbSMS_GW,iDummy)!=0) ) ){
#else
                (strncmp((CHAR*) vDummy,pNetwork->pbSMS_GW,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		vDummy = getMyPtrFromMem( dbChannelRef, (const BYTE *)USSD_C, FALSE, &uiErrorCode, &iDummy );
#else
        vDummy = getMyPtrFromMem( dbChannelRef, USSD_C, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
				(strncmp((CHAR*) vDummy,(char *)pNetwork->pbUSSD_CENTER,iDummy)!=0) ) ){
#else
                (strncmp((CHAR*) vDummy,pNetwork->pbUSSD_CENTER,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		iDummy = (int)db_getInt( dbChannelRef, (const BYTE *)USSD_GW_TYPE, FALSE, &uiErrorCode );
#else
        iDummy = (int)db_getInt( dbChannelRef, USSD_GW_TYPE, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		vDummy = getMyPtrFromMem( dbChannelRef, (const BYTE *)USSD_GW, FALSE, &uiErrorCode, &iDummy );
#else
        vDummy = getMyPtrFromMem( dbChannelRef, USSD_GW, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
				(strncmp((CHAR*) vDummy,(char *)pNetwork->pbUSSD_SC,iDummy)!=0) ) ){
#else
                (strncmp((CHAR*) vDummy,pNetwork->pbUSSD_SC,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
		iDummy = (int)db_getInt( dbChannelRef, (const BYTE *)WAP_TIMEOUT, FALSE, &uiErrorCode );
#else
        iDummy = (int)db_getInt( dbChannelRef, TIMEOUT, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
			vDummy = getMyPtrFromMem( dbChannelRef, (const BYTE *)CREDENTIALS, FALSE, &uiErrorCode, &iDummy );
#else
            vDummy = getMyPtrFromMem( dbChannelRef, CREDENTIALS, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
					(strncmp((CHAR*) vDummy,(char *)pUaSet->pbProxyCredentials,iDummy)!=0) ) ){
#else
                    (strncmp((CHAR*) vDummy,pUaSet->pbProxyCredentials,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
			vDummy = getMyPtrFromMem( myDB_Root, (const BYTE *)WSP_LANGUAGE, FALSE, &uiErrorCode, &iDummy );
#else
            vDummy = getMyPtrFromMem( myDB_Root, WSP_LANGUAGE, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
					(strncmp((CHAR*) vDummy,(char *)pUaSet->pbLanguage,iDummy)!=0) ) ){
#else
                    (strncmp((CHAR*) vDummy,pUaSet->pbLanguage,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
		...
		if( pvWAEMaStruct != NULL ){
			pWAEMainStruct = (pWAEMAINOBJECT)pvWAEMaStruct;
#ifdef FEATURE_WAP
			bDummy = (BYTE*)db_getPtr( myDB_Root, (const BYTE *)USER_AGENT, FALSE, &uiErrorCode );
#else
            bDummy = (BYTE*)db_getPtr( myDB_Root, USER_AGENT, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP
			...
			if( bDummy != NULL ){
				...
				DEALLOC( &(pWAEMainStruct->pbUSERAGENT) );
#ifdef FEATURE_WAP
			bDummy = (BYTE*)db_getPtr( myDB_Root, (const BYTE *)PROFILE, FALSE, &uiErrorCode );
#else
            bDummy = (BYTE*)db_getPtr( myDB_Root, PROFILE, FALSE, &uiErrorCode );
#endif //#ifdef FEATURE_WAP
			...
			if( bDummy != NULL ){
				...
				DEALLOC( &(pWAEMainStruct->pbPROFILE) );
#ifdef FEATURE_WAP
			vDummy = getMyPtrFromMem( myDB_Root, (const BYTE *)PROFILE_DIFF, FALSE, &uiErrorCode, &iDummy );
#else
            vDummy = getMyPtrFromMem( myDB_Root, PROFILE_DIFF, FALSE, &uiErrorCode, &iDummy );
#endif //#ifdef FEATURE_WAP
...
#ifdef FEATURE_WAP
					(strncmp((CHAR*) vDummy,(char *)pWAEMainStruct->pbPROFILE_DIFF,iDummy)!=0) ) ){
#else
                    (strncmp((CHAR*) vDummy,pWAEMainStruct->pbPROFILE_DIFF,iDummy)!=0) ) ){
#endif //#ifdef FEATURE_WAP
...
BYTE getContentTypeByte( void* pvHeaderHandle )
{
	...
	pHeaderList = (pHEADERDEF)(pvHeaderHandle);
	if( pHeaderList->pHeadList != NULL ){
		pHeaderElement = WSP_GetHeaderWK ( 1, Field_Content_Type, pHeaderList->pHeadList );
		if (pHeaderElement != NULL){
			bContentType = WSP_GetContentTypeByte (pHeaderElement, &fError, & pWSPParam);
			if( pWSPParam )
			...
			int iOptions, int iPrio, unsigned long int uiPid, char chWaitingFor )
{
	...
	if( pvTransList != NULL ){
		...
		if( pTransStruct != NULL ){
			...
			pTransStruct->chWaitingFor = chWaitingFor;
	...
	if( pvTransList != NULL ){
		...
		while( pvListElement != NULL ){
			...
			if( pvElementContenet != NULL ){
				...
				if( pTransStruct != NULL ){
					if( pTransStruct->iObjectID == iObjectID && pTransStruct->chWaitingFor == TR_LIST_WAIT_FOR_CHANNEL ){
						return 1;
					}
				}
			}
...
int TR_List_ClearWaitingForE2E( void* pvTransList, int iTID )
{
	...
	if( pvTransList != NULL ){
		...
		if( pvListElement != NULL ){
			pvElementContent = SDL_GetListElementContent( pvListElement );
			if( pvElementContent != NULL ){
				pTransStruct = (TRANSACTIONSTRUCT*)pvElementContent;
				if( pTransStruct != NULL ){
					if( pTransStruct->chWaitingFor == TR_LIST_WAIT_FOR_E2E ){
						iSID = getDataChannel( (UINT8)pTransStruct->iObjectID, ((pURLREQSTRUCT)(pTransStruct->pvReqStruct))->pbURL, &bExpired, &iError );
						if( iError == DB_err_success ){
							pTransStruct->iSID = iSID;
							pTransStruct->chWaitingFor = 0;
							iReturn = 1;
						}
					}
				}
	...
	return iReturn;
} /* end of TR_List_ClearWaitingForE2E */
	...
	if( pvTransList != NULL ){
		...
		while( pvListElement != NULL ){
			...
			if( pvElementContenet != NULL ){
				...
				if( pTransStruct != NULL ){
					if( pTransStruct->iObjectID == iObjectID && pTransStruct->chWaitingFor == TR_LIST_WAIT_FOR_CHANNEL ){
						iSID = getDataChannel( (UINT8)iObjectID, ((pURLREQSTRUCT)(pTransStruct->pvReqStruct))->pbURL, &bExpired, &iError );
						if( iError == DB_err_success ){
							pTransStruct->iSID = iSID;
							pTransStruct->chWaitingFor = 0;
						}
	...
	void** pvReqStruct, long int* liOptions,	long int* liPrio, unsigned long int* uiPid )
{
	...
	if( pvTransList != NULL ){
		...
		while( pvListElement != NULL ){
			...
			if( pvElementContenet != NULL ){
				pTransStruct = (TRANSACTIONSTRUCT*)pvElementContenet;
				if( pTransStruct != NULL && !(pTransStruct->chWaitingFor) ){
	...
	if( pvTransList != NULL ){
		...
		while( pvListElement != NULL ){
			...
			if( pvElementContenet != NULL ){
				pTransStruct = (TRANSACTIONSTRUCT*)pvElementContenet;
				if( pTransStruct != NULL ){
					if( pTransStruct->iObjectID == iObjectID && pTransStruct->iSID == iSID && !(pTransStruct->chWaitingFor) ){
						*liTID = (long int)SDL_GetListElementID( pvListElement );
						*pvReqStruct = pTransStruct->pvReqStruct;
						*liOptions = (long int)pTransStruct->iOptions;
						*liPrio = (long int)pTransStruct->iPrio;
...
/*========================================================================
	Check_If_Multipart_Push
==========================================================================
Purpose: Parse the Headers and check if Multipart/mixed or Connectivity. VoidStar will after the function points at the start of the headerlist
Params:  pvHeader, iHeaderLength
Return:  -1, Error in Pre-parse
		 0, if no Multipart
		 1, if Multipart/mixed
		 2, if Multipart/related
		 3, if Multipart/alternative
=========================================================================*/
int Check_If_Multipart_Push( void* vsPushHeader, UINT32 iLen, void** vsHeaderHandle, UINT16 *iSEC, void** vsMAC )
{
	...
	{
		/* Error in pre-parse return -1 */
		pHeaderDef=NULL;
		iReturn = -1;
	}
	else
	{
		*vsHeaderHandle = (void*)pHeaderDef;
		fError = FALSE;
		pElement=WSP_GetHeaderWK(1,Field_Content_Type,pHeaderDef->pHeadList);
		if( pElement != NULL ){
			bContentType = WSP_GetContentTypeByte( pElement, &fError, &pParameters );
				
			if( !fError && bContentType == 0x23 ) /* ContentType is application/vnd.wap.multipart.mixed */
				iReturn = Multipart_Mixed;
			else if( !fError && bContentType == 0x33 ){
			/* ContentType is application/vnd.wap.multipart.related. Don't support this today dealloc pHeaderDef */
				iReturn = Multipart_Related;
				if( pHeaderDef != NULL )
					DeleteHEADERDEF( pHeaderDef );
			}
			else if( !fError && bContentType == 0x26 ){  /* ContentType is application/vnd.wap.multipart.alternativ. Don't support this today dealloc pHeaderDef */
				iReturn = Multipart_Alternative;
				if( pHeaderDef != NULL )
					DeleteHEADERDEF( pHeaderDef );
			}
			else if (!fError && bContentType == 0x36){ /* Content type is connectivity-wbxml*/
				iReturn = Connectivity;
				if (pParameters != NULL)
				{
					*iSEC = (UINT8)pParameters->iSEC;
					*vsMAC = (void*) B_CopyByteString ((BYTE*)pParameters->pbMAC, -1);
				}
				else
				{
					*iSEC = 255;
	...
	return iReturn;
} /* end Check_If_Multipart_Push */

/*========================================================================
	Get_Multipart_Url
==========================================================================
Purpose: Get the Url defined at Multipart level
Params:  vsDummy (pointer to the list of the parsed multipart headers)
Return:  void* (pointer to the Url. Returns NULL if no Url is defined)
=========================================================================*/
void* Get_Multipart_Url(void* vsDummy)
{
	pHEADERDEF pHeaderDef = (pHEADERDEF)vsDummy;

	return (void*) Get_Url(pHeaderDef);
} /* end Get_Multipart_Url */

/*========================================================================
	PreParse_Multipart_Body
==========================================================================
Purpose: Pre-parse the multipart body into entitys
Params:  vsBody, iBodyLen (pointer to the body of the multipart package, length of the body) 
Return:  void* (returns a pointer to a list of MULTIPARTSTR including all the entitys in the multipart body)
=========================================================================*/
void* PreParse_Multipart_Body(void* vsBody, int iBodyLen)
{
	return (void*) WSP_PreParseMultipart( (BYTE*)(vsBody), iBodyLen );  
} /* end PreParse_Multipart_Body */


/*========================================================================
	Check_If_Entity_Is_PushMessage
==========================================================================
Purpose: 
Params:  
Return:  
...
int Check_If_Entity_Is_PushMessage(void* vsEntityList, void* vsHeaderHandle, int iCacheInIdle, void** vsEntityHeader, UINT32 *iHeaderLength, void** vsEntityBody, UINT32 *iBodyLen)
{
	...
	pMultiPartList = (pMULTIPARTSTR) vsEntityList;
	pbPushHeader = (BYTE*) pMultiPartList->pbHdrStart;
	pHeaderDef=WSP_PreParseHeaders(pbPushHeader,pMultiPartList->iHdrLen,&fError);   
	pElement=WSP_GetHeaderWK(1,Field_Content_Type,pHeaderDef->pHeadList);
	if( pElement != NULL ){
			bContentType = WSP_GetContentTypeByte( pElement, &fError, &pParameters );
			if ((!fError && ( bContentType == 0x1C || bContentType == 0x1D || bContentType == 0x1E || bContentType == 0x1F
				|| bContentType == 0x20 || bContentType == 0x21 || bContentType == 0x14 || bContentType == 0x15)))
			{
				Write_To_Cache_If_Ok( pHeaderDef, vsHeaderHandle, iCacheInIdle, pMultiPartList);
				iTemp = 0;
			}
			else
			{
				pbString = NEWARRAY(BYTE,pMultiPartList->iHdrLen+1);
				B_COPYSTRINGN(pbString,pMultiPartList->pbHdrStart, pMultiPartList->iHdrLen); 
				*vsEntityHeader = (void*) pbString;
				pbString1 = NEWARRAY(BYTE,pMultiPartList->iBodyLen+1);
				B_COPYSTRINGN(pbString1,pMultiPartList->pbBodyStart, pMultiPartList->iBodyLen);
				*vsEntityBody = (void*) pbString1;
				*iHeaderLength = pMultiPartList->iHdrLen;
				*iBodyLen = pMultiPartList->iBodyLen;
				iTemp = 1;
			}
	}
	if( pHeaderDef != NULL )
			DeleteHEADERDEF( pHeaderDef );
	return iTemp;
} /* end Check_If_Entity_Is_PushMessage */
				
/*========================================================================
	Write_To_Cache_If_Ok
==========================================================================
Purpose: 
Params:  
Return:  
...
void Write_To_Cache_If_Ok(pHEADERDEF pHeaderDef, void* vsHeaderHandle, int iCacheInIdle, pMULTIPARTSTR pMultiPartList)
{
	...
	pHEADERELEMENT pElement = NULL;
	BYTE* StoreUrl = NULL;
	BYTE* MultipartUrl = NULL;
	BYTE* EntityUrl;

	iCacheControl = GetCacheControl(pHeaderDef->pHeadList);
	if( !(iCacheControl == NoStore || iCacheControl == Private || iCacheInIdle == 0) )
	{
		pElement = WSP_GetHeaderWK(1,Field_X_Wap_Content_URI,pHeaderDef->pHeadList);
		if (pElement != NULL)  /* if x-wap-content-uri is specifed store under this URI */
		{
			WriteContentIntoCache( WSP_GetXContentURI(pElement), (void*) pMultiPartList->pbHdrStart, pMultiPartList->iHdrLen, 
					(void*) pMultiPartList->pbBodyStart, pMultiPartList->iBodyLen, WML_USER_AGENT);
		}
		else
		{
			EntityUrl = Get_Url(pHeaderDef);
			if (b_UrlIsCompleteValid (EntityUrl))  /* if content-location is specified and absolute, store under this uri */
				WriteContentIntoCache( EntityUrl, (void*) pMultiPartList->pbHdrStart, pMultiPartList->iHdrLen, 
					(void*) pMultiPartList->pbBodyStart, pMultiPartList->iBodyLen, WML_USER_AGENT);
			else
			{
				MultipartUrl = (BYTE*) Get_Multipart_Url(vsHeaderHandle);
				if (b_Resolve((BYTE*)MultipartUrl, EntityUrl, &StoreUrl) && b_UrlIsCompleteValid (StoreUrl))
				{ /* if b_resolve on content-location for multipart and content-location for entity is absolute, store under this uri*/
					WriteContentIntoCache( StoreUrl, (void*) pMultiPartList->pbHdrStart, pMultiPartList->iHdrLen, 
						(void*) pMultiPartList->pbBodyStart,  pMultiPartList->iBodyLen, WML_USER_AGENT);
					DEALLOC(&StoreUrl);
				}
			}	
...
BYTE* Get_Url(pHEADERDEF pHeaderDef)
{
	...
	pElement = WSP_GetHeaderWK(1,Field_Content_Location,pHeaderDef->pHeadList);
	if (pElement != NULL)
		return WSP_GetContentLocation(pElement);
	else
		return NULL;
}
...
{
	pMULTIPARTSTR	pMultiPartList = (pMULTIPARTSTR) vsEntityList;
	pMULTIPARTSTR pTemp = NULL;

	pTemp = pMultiPartList;
	vsEntityList = (void*) pMultiPartList->pNext;
	DEALLOC(&pTemp);
	if (vsEntityList != NULL)
		*iTemp = 1;
	else{
		if( vsHeaderHandle != NULL )
			DeleteHEADERDEF( (pHEADERDEF) vsHeaderHandle);
		*iTemp = 0;
	}
	return vsEntityList;
}  /* end Get_Next_Entity */
...
int SE_List_Find_An_Object_For_Push( void* pvSessionList )
{
	...
	if( pvSessionList != NULL ){
		...
		while( pvListElement != NULL ){
			iObjectID = SDL_GetListElementID( pvListElement );
			if( iObjectID == CONTENT_USER_AGENT || iObjectID == CONTENT_USER_AGENT
					|| iObjectID == CONTENT_USER_AGENT )
				pvListElement = SDL_GetNextListElement( pvSessionList, pvListElement );
			else
				return iObjectID;
		}
	...
	return -1;
} /* end of SE_List_Find_An_Object_For_Push */
		...
		if( WSP_ParseAddress( pbAddress, iAddressLength, &pAddressList ) ){
			...
			if( pAddressList != NULL ){
				if ( pAddressList->iPortNumber == -1)
					pAddressList->iPortNumber = MODE_CO_WSP;
	...
	*pliHeaderLength = 0;
	...
	if( NR_OF_ACCEPTCHARSET > 0 ){
		for ( i=0; i<NR_OF_ACCEPTCHARSET; i++ ){
			if( arrayAcceptCharset[i] < 256 ){
				tempArray[0] = arrayAcceptCharset[i] & 0xff;
						/* GWC sends this header in Encoding version 1.2, propably minor trouble with older Gateways. */
				WSP_CreateHeaderField( Field_Accept_Charset, tempArray, 1, &pList);
			}
			else{
				tempArray[0] = (arrayAcceptCharset[i] >> 8) & 0xff;
...
int CheckServerHeaders( VOID* pvServerHeaders, int iLen, long int* pliEncodingVersion)
{
	...
	*pliEncodingVersion = ENCODING_VERSION_1_2;
			...
			if( pHeaderList->pHeadList != NULL )
			{
				...
				if( pHeaderElement != NULL ){
					...
					if( bEncodingVersion != 0 )
						*pliEncodingVersion = bEncodingVersion;

				}
				if( *pliEncodingVersion == ENCODING_VERSION_1_2 || *pliEncodingVersion == ENCODING_VERSION_1_3 )
					pHeaderElement = WSP_GetHeaderWK ( 1, Field_Profile_Warning, pHeaderList->pHeadList );
				else
					pHeaderElement = WSP_GetHeaderWK ( 1, Field_Profile_Warning_1_4, pHeaderList->pHeadList );
				if( pHeaderElement != NULL )
...
// B1»óÅÂ·Î ¼öÁ¤ÇÔ !! --> ´Ù½Ã B2·Î °íÃÄ¾ßÇÔ 
static void MakeB5_info(void)
{
	BYTE* pAgent = agent;
	...
	*pAgent++ = CELLULAR_95C;
	...
	strncpy((char *)pAgent,(const char *)IM_3300,2);
	pAgent+=2;	
	brow_length+=2;
	...
	strncpy((char *)pAgent,(const char *)LCDPIXCEL_B2,6);
	pAgent+=6;	
	brow_length+=6;
	
	strncpy((char *)pAgent,(const char *)LCDSIZE_B2,4);
	pAgent+=4;	
	brow_length+=4;
	
	strncpy((char *)pAgent,(const char *)LCDCOLOR_65,2);
	pAgent+=2;	
	brow_length+=2;
