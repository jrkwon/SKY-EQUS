...
#ifdef FEATURE_WAP
#define WIP_MALLOC_MEM_SIZE 75000
#else
#define WIP_MALLOC_MEM_SIZE 25000
...
/*
 * The Cache, the Push Repository, the WTA Service Repository,
 * and the Configuration Database, can either use a memory-based
 * implementation or a file-based one. Default is to use
 * the memory-based version. If you want to use the file version
 * instead, define the corresponding symbol below.
 *
 * (Currently only the Cache and Push Repository can use
 *  a file-based version. Leave the other symbols undefined!)
 */
/* #define FILE_CACHE */
/* #define FILE_PUSH_REPOSITORY */

/* #define FILE_WTA_REPOSITORY */
/* #define FILE_CONFIG_DATABASE */

#define USE_STORAGE (!defined (FILE_CACHE) \
                     || !defined (FILE_PUSH_REPOSITORY) \
                     || !defined (FILE_WTA_REPOSITORY) \
                     || !defined (FILE_CONFIG_DATABASE))
...
typedef enum ConfigVars
{
    ...
    cfg_wae_wspif_CookieItems  = 4,          /* maximum number of items GWC stores in the Cookie list */
	...
	cfg_wae_wspif_ReConnectOnDisconnect = 1, /* Handling when the GW disconnect a WSP-session: 0:Generate an error to the application. 1:Reconnect the WSP-session immediately. */
	cfg_wae_wspif_MaxOutstandingRequest = 5, /* indicate the maximum number of simultaneously outstanding request. Allow values are between 1 and 5. */
    /* wae wspif end ---------------------------------------------------------*/
    ...
    /* WTP -------------------------------------------------------------------*/
    no_of_retransmissions_UDP_WTP = 4,//8
    retransmission_interval_UDP_WTP = 100, //50,
    ...
    WTP_SAR_GROUP_SIZE = 10240,
    //WTP_SAR_SEGMENT_SIZE = 1024,
    WTP_SAR_SEGMENT_SIZE = 5120,
    /* WTP end ---------------------------------------------------------------*/
...
#define ACCEPT_CHACTERBELL "application/x-cmf"
#else
...
#define LCDPIXCEL_B2  "128160"

#define LCDSIZE    "1605" 
#define LCDSIZE_B1    "2008"
#define LCDSIZE_B2    "1809"

#define LCDCOLOR   "02"
#define LCDCOLOR_MONO  "01"
#define LCDCOLOR_256   "08"
#define LCDCOLOR_65    "16"
#endif //#ifdef FEATURE_WAP
...
// memory warning 2001/09/29 ytchoi
//#define PARSE_MODE 1
