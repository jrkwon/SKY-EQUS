...
BOOL g_EIF_WAP_GVMINFO = FALSE;				// for gvminfo_command

BOOL g_EIF_WAP_WAPCON = FALSE;				// for wapcon_command
...
void resultTokening_output(char* pSrc, int nLen)
{
	...
	while (nLen > 0) 
	{
		if (count >= 56)		// tokenized result output
		{
			*pDest = '\0';
...
void iwfcon_command(void)
{
	...
	if(destinationIP != 0)//if(wapip != 0)		// WAP connection OK!!
	{
		for (loop=0 ; loop<4 ; loop++)
		{
			phoneIP[4 - loop -1] = ((char*)&Ip_addr)[loop];
			gatewayIP[4 - loop -1] = ((char*)&destinationIP)[loop];
...
/*===========================================================================
===========================================================================*/
void wapcon_command(void)
{
	extern void EIF_wapcon_connection(void);
	EIF_wapcon_connection();
}
/*===========================================================================
...
LOCAL UINT16 EIF_sGetFormatLength(BYTE *pszFormat, UINT16 nLength, UINT8 nFormatLen)
{
    UINT16   nLen, i, j;
...
LOCAL UINT16 EIF_sGetStringLength(UINT16 nLen)
{
    UINT16   nStringLen, i;
    ...
    for(i=0; i < nLen; i++)
    {
        if(IsMiddleOfMultiByte(token, (UINT16)(i+1)))
            i++;
        nStringLen ++;
    }
...
LOCAL TEXT_FORMAT_TYPE_E EIF_sGetCurrentFormat(BYTE *pszFormat, UINT16 nLen)
{
    ...
    nStringLen = EIF_sGetStringLength(nLen);
...
LOCAL BOOL EIF_sCheckFormatString(BYTE *pszFormat, int nChars)
{
    int nLen, i, nOffset;
    TEXT_FORMAT_TYPE_E Type;
    BYTE *szTmp, nTmp;

    nLen = EIF_sGetFormatLength(pszFormat, STRLEN(pszFormat), 0);
	if(nChars < nLen) // pInput->nChars < input string길이
		nLen = nChars;

    if(EIF_sGetStringLength(STRLEN(token)) > nLen)//if(EIF_sGetStringLength(pTextEdit->nLen > nLen) > nLen)
        return FALSE;

    for(i = 0; i < STRLEN(token) ;)//for(i = 0; i < pTextEdit->nLen ;)
    {
        Type = EIF_sGetCurrentFormat(pszFormat,i);

        switch(Type){
        case TFT_A:
            if(token[i] < 'A' || token[i] > 'Z')  // Upper Only
                return FALSE;
            break;

        case TFT_a:
            if(token[i] < 'a' || token[i] > 'z')  // Lower Only
                return FALSE;
            break;

        case TFT_N:
            if(token[i] < '0' || token[i] > '9')  // Number Only
                return FALSE;
            break;

        case TFT_X:
            if(token[i] >= '0' && token[i] <= '9')    // Not Number
                return FALSE;

            if(token[i] >= 'a' && token[i] <= 'z')    // Not Lower
                return FALSE;
            break;
        ...
        case TFT_x:
            ...
            if(token[i] >= 'A' && token[i] <= 'Z')    // Not Upper
                return FALSE;
            break;
        ...
        while(TRUE)
        {
            szTmp = STRCHR((BYTE*)(pszFormat + nOffset), '\\');
            if(szTmp == NULL)
                break;

            nTmp = szTmp - pszFormat;
            nLen = EIF_sGetFormatLength(pszFormat, nTmp, 0);
            nOffset = nTmp+1;
...
int Check_input_Error(char *String)
{
	WAP_Node *pCurNode;
	int string_num = 0, input_num = 0;
	int length;
	BOOL result = FALSE;
	int i;	
	...
	while(pCurNode != NULL)
	{
		if(pCurNode->type == _ELEMENT_TYPE_INPUT)
		{
			...
			input_num ++; //실제 g_pCard에 있는 input의 갯수
			//if(String != NULL)	string_num ++;	//sting으로 들어온 input의 갯수

			memset(token, '\0', WAP_MAX_INPUT+1);
			String = getStringToken(String, (char*)token);					// INPUT 명령에 실제 필요한 token을 얻는다.
			length = get_Stringlength(token, WAP_MAX_INPUT);

			//입력을 원하지 않는 경우 check!! - ``으로 구분되는 경우
			if(length == 0)	
			{
				if(pInput->emptyOk == FALSE)	//입력된 string없으나, 꼭 입력을 해야하는 경우
					return FALSE;	//Error!!
			}
			else
			{
				if(pInput->format != NULL)
				{
					if(pInput->nChars > 0)
					{ 
						result = EIF_sCheckFormatString((BYTE *)pInput->format, pInput->nChars);//CheckFormatString(pInput->format, token, pInput->nChars);
					}
					else
					{ 
						result = EIF_sCheckFormatString((BYTE *)pInput->format, WAP_MAX_INPUT);//CheckFormatString(pInput->format, token, pInput->nChars);
					}
	...
	if(string_num != input_num) //string갯수가 다른 경우 Error!!
	{
		return -1;	//개수가 다른 경우 Error
	}
	return 1;	//성공
}
...
void input_command(char *InputString)
{
	...
	if(destinationIP != 0)//if ((wapip != 0))//&&(SKY_IsInCall()))//if ((wapip != 0)&&(ui.call))
	{
		...
		result = Check_input_Error(InputString);

		if(result == 0)//FALSE)
		{
			at_send_extended_text("*SKT*INPUT:0");		
		}
		else if(result == -1)	//INPUT갯수가 다른 경우 ERROR처
		{
			at_send_extended_text("ERROR");
		}
		else if(result == 1)
		{
			pCurNode = g_pCard->pHeadFocus;		// 포커스 이동을 위해 포인터를 세팅 
...
void uopen_command(char *WantToRead_URL)
{
	...
	if (strstr(temp_url, "http") != NULL)	// URL has header.
	{
		...
		SKY_loadURL(VIEW_ID_WML, WantToRead_URL, TRUE);
	...
	else									// URL has not header
	{
		...
		SKY_loadURL(VIEW_ID_WML, url_plus_header, TRUE);
	}
...
void gvminfo_command(int number)
{
	memset(resultBuf, 0x00, sizeof(resultBuf));

	if (number == 0)			// GVMINFO 해제 
	{
		char * result = "*SKT*GVMINFO:1";
		g_EIF_WAP_GVMINFO = FALSE;
	...
	else if(number == 1)	// GVMINFO 설정 
	{		
		char * result = "*SKT*GVMINFO:1";
		g_EIF_WAP_GVMINFO = TRUE;
...
void eif_error_command(int Kind_of_error)
{
	switch(Kind_of_error)
	{
		...
		case EIF_ERR_UPING_CONNECT_FAIL:
			...
			break;
		case EIF_ERR_WAPCON_NOT_IN_WAP:
			at_send_extended_text("*SKT*WAPCON:Connection Failed [Phone is in idling]");
			break;
		case EIF_ERR_WAPCON_CONNECT_FAIL:
			at_send_extended_text("*SKT*WAPCON:Connection Failed [Network connecting failure]");
			break;
	}
