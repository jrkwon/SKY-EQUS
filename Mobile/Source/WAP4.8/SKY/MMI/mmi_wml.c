...
#include "wapcommon.h"	//yjahn96 01.11.09
...
point Calculate_Node_StartPoint(WAP_Node* pPrevNode, WAP_Node* pCurNode)
{
	...
	{
		//yjahn96 01.12.04 ------------------------------------------------------->
		//남은 여백에 실제 글자를 표시할수 없는 경우(한글인 경우), 새라인으로 조정
		char *pCurTextData;
		pCurTextData = GetCurDisplayedData(pCurNode);

		if(*pCurTextData & 0x80)	//한글이면서
		{
			if(WAP_LCD_WIDTH - pPrevNode->x2 < pCurNode->font.width*2)//한글자를 표시할수 없는 경우
				result.x = 0;
			else
		...
		if(pPrevNode->font.font_index == WAP_FONT_MEDIUM)
		{
			...
			while(pNode != NULL && pNode->y2 - pNode->font.height == pPrevNode->y1)
			{
				...
				{
					result.y = pPrevNode->y2 - pPrevNode->font.height + (WAP_ASCII_FONT_HEIGHT - WAP_MED_ASCII_FONT_HEIGHT);//+3
					return (result);
				}
				pNode = pNode->pPrevNode;
			}
...
*/
//yjahn96 01.12.05 -----------------------------------------------------------------
//font가 mix되어 있는 경우, 연속되는 라인에서 남은 여백이 한글을 쓸 만큼은 남아 있지 않을 때
//새 라인으로 이동하도록 수정 - formatting시 처리
//----------------------------------------------------------------------------------
point Calculate_Node_StartPoint(WAP_Node* pPrevNode, WAP_Node* pCurNode)
{
	point result;
	...
	else							// 연속된 라인
	{
		...
		if(*pCurTextData & 0x80)	//한글이면서
		{
			if(WAP_LCD_WIDTH - pPrevNode->x2 < pCurNode->font.width*2)//한글자를 표시할수 없는 경우
			{
				...
				if(pPrevNode->font.font_index == WAP_FONT_MEDIUM)
				{
					...
					{
						if((pNode->font.font_index == WAP_FONT_NORMAL) 
							&&((pPrevNode->y2 - pPrevNode->font.height) == pPrevNode->y1)) //PreNode와 CurNode가 같은 라인에 있고, 그 이전노드에 Normal이 있는 경우
						{
							//result.y = pPrevNode->y2 - pPrevNode->font.height + (WAP_ASCII_FONT_HEIGHT - WAP_MED_ASCII_FONT_HEIGHT);//+3
							result.y = pPrevNode->y2  + (WAP_ASCII_FONT_HEIGHT - WAP_MED_ASCII_FONT_HEIGHT);//+3
							return (result);
		...
		if(pPrevNode->font.font_index == WAP_FONT_MEDIUM)
		{
			...
			while(pNode != NULL && pNode->y2 - pNode->font.height == pPrevNode->y1)
			{
				if((pNode->font.font_index == WAP_FONT_NORMAL) //PreNode(->x1)와  같은 라인에 Normal존재
					&&((pPrevNode->y2 - pPrevNode->font.height) != pPrevNode->y1))// CurNode(->x1)와 PrevNode(->x1)가 같은 line에 있지 않은 경우 해당
				{
					result.y = pPrevNode->y2 - pPrevNode->font.height + (WAP_ASCII_FONT_HEIGHT - WAP_MED_ASCII_FONT_HEIGHT);//+3
					return (result);
				}
...
void WAP_Send_LCDBuffer_ToHS(void)
{
    ...
    //SKY_RefreshDisplayRect((BYTE *)GetScreenBuffer(), 0, WAP_HEAD_HEIGHT-2,	WAP_LCD_TOTAL_ADDRESS_X-1, WAP_LCD_TOTAL_ADDRESS_Y-1);
}
...
void WAP_Display(void)
{
	...
	extern HCONTROL                    hNTopAnim ;	//초기 animation stop위해
    ...
    MSG_HIGH(" WAP_Display() is called!! ",  0, 0, 0);
    
	//NATE 초기 접속시 초기 card받으면 animation control없앰
...
//#ifdef WAP_USE_SKT_NEW
    ...
    {
        WAP_DrawLoadingBarToBackBuff(WAP_ANI_TEXT_RECEIVING, WAP_ANI_BAR_FROM_LEFT);
    }
//#endif//#ifdef WAP_USE_SKT_NEW
...
void WAP_DrawPartialUpdate(WAP_Node *pCurNode)
{
	switch(pCurNode->type)
	{
		case _ELEMENT_TYPE_TEXT  :
...
void WAP_Display_Partial(BOOL bDown)
{
	extern HCONTROL hContentsDownloadControl;
    ...
    {
        if(g_pCurFocusNode->pPrevFocusNode == NULL) // 마지막 focus node -> 첫번째 focus node(현재)
        {
            //1. Update for the last focus node 
            pFirstFocusNode = g_pCard->pHeadFocus;
			while(pFirstFocusNode != NULL)
			{
				if(pFirstFocusNode->pNextFocusNode !=NULL)
					pFirstFocusNode = (WAP_Node *)pFirstFocusNode->pNextFocusNode;
				else
					break;
			}

            WAP_DrawPartialUpdate(pFirstFocusNode);

            x1 = 0;//pFirstFocusNode->x1;
            y1 = (pFirstFocusNode->y1 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
            x2 = WAP_LCD_TOTAL_ADDRESS_X-1;
            y2 = (pFirstFocusNode->y2 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
		    //handset task에 display를 요청한다
		    SKY_RefreshDisplayRect((BYTE *)GetScreenBuffer(), x1, y1, x2, y2);//WAP_Send_LCDBuffer_ToHS();

            
            //2. Update for the first focus node 
            pSecondFocusNode = g_pCurFocusNode;

            WAP_DrawPartialUpdate(pSecondFocusNode);

            x1 = 0;//pFirstFocusNode->x1;
            y1 = (pSecondFocusNode->y1 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
            x2 = WAP_LCD_TOTAL_ADDRESS_X-1;
            y2 = (pSecondFocusNode->y2 - g_nLCDOffset) + WAP_HEAD_HEIGHT;//(pSecondFocusNode->y1 - g_nLCDOffset) + WAP_HEAD_HEIGHT + pSecondFocusNode->font.height;;
		    //handset task에 display를 요청한다
		    SKY_RefreshDisplayRect((BYTE *)GetScreenBuffer(), x1, y1, x2, y2);//WAP_Send_LCDBuffer_ToHS();
        ...
        {
            pFirstFocusNode = g_pCurFocusNode->pPrevFocusNode;
            pSecondFocusNode = g_pCurFocusNode;
    ...
    {
        if(g_pCurFocusNode->pNextFocusNode == NULL)    // 첫번째 focus node -> 마지막 focus node(현재)
        {
            //1. Update for the last focus node
            pFirstFocusNode = g_pCurFocusNode;
            ...
            //2. Update for the first focus node
            pSecondFocusNode = g_pCard->pHeadFocus;
            ...
            y1 = (pSecondFocusNode->y1 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
            x2 = WAP_LCD_TOTAL_ADDRESS_X-1;
            y2 = (pSecondFocusNode->y2 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
		    //handset task에 display를 요청한다
		    SKY_RefreshDisplayRect((BYTE *)GetScreenBuffer(), x1, y1, x2, y2);//WAP_Send_LCDBuffer_ToHS();
        ...
        {
            pFirstFocusNode = g_pCurFocusNode;
            pSecondFocusNode = g_pCurFocusNode->pNextFocusNode;
...
void WAP_Scrolling(BOOL bDown)
{
	...
	{
        WAP_Display_Partial(bDown);//WAP_Display();
	}
	//------------------------------------------------------------------
...
BOOL WAP_FocusScrolling_Down(void)
{
	...
	if(pNextFocusNode == NULL){
		...
		{
			//------------------------------------------------------------
			// Element갯수가 LCD의 최대 display Line수(8) 보다 작은 경우,
			// Scroll down시 화면이동은 하지 않는다.  -- yjahn96 00.12.20
			//------------------------------------------------------------
...
#ifdef	WAP_NOWRAP
char tmpText[WAP_MAX_TEXT/(WAP_HS_SCRN_WIDE-2)+2][WAP_HS_SCRN_WIDE-1];	//16자씩 넣을 수 있는 (WAP_HS_SCRN_WIDE-1)byte짜리 buffer 갯수 : (256/16)+1 = 17 
		...
		while(pCurNode != NULL)
		{
			...
			&& (pCurNode->pNextNode == NULL || pCurNode->pNextNode->y1 > g_nLCDOffset))
			{
				if(pCurNode->font.font_index == WAP_FONT_MEDIUM)
				{
					//----------------------
					//for dual fonts
					//	현재 노드가 Normal Font일 경우, 현재 노드 뿐 아니라
					//	현재 노드와 같은 라인에 Big Font가 존재하는 지 확인해야 한다
					...
					if(bMixFontLine)
						g_nLCDOffset += WAP_ASCII_FONT_HEIGHT;//16
					else
...
BOOL WAP_WindowScrolling_Up(void)
{
	...
	if(g_nLCDOffset == 0)
	{
		...
		if(g_pCard ->isKeyCard)
		{
            ...
            if(g_nLCDOffset < 0)    //card만 있는 경우, scroll안함..
                return FALSE;
            else
			    return TRUE;
		}
...
/*			WAP_Display_Rectangle(pMe->x1, 
								    ...
								    WAP_BLACK);	//color
*/
			    WAP_Display_TextOut(x, y, pStr,//WAP_Display_TextOut_Focus(x, y, pStr,
								    pMe->font.font_index, nDrawMode, pText->isLink, nAlign,
								    nTableColumns, nCurColumn);//for Table
		    }
...
void WAP_Draw_Element_Key(WAP_Node *pMe)
{
	...
	{
		memset(label, '\0', WAP_MAX_TEXT);
		strncpy(label, tmpText[curNoWrap],WAP_HS_SCRN_WIDE-2);    //16자
	}
#endif//#ifdef	WAP_NOWRAP
...
boolean Check_Play_SIS_Animation(void)
{
	...
	if(!( g_nViewMode == WAP_VIEW_MODE_WML &&
        ...
        return FALSE;   //WML일 경우에만 timer set
	...
	while(pCurNode != NULL)
	{
		...
		|| pCurNode->type == _ELEMENT_TYPE_LINK_IMAGE)
		{
			...
			if(pImage->imageType == WAP_IMAGE_TYPE_SIS)
			{
				...
				if(result == 0)
				{
					pImage->width	= WAP_LCD_WIDTH-1;
					pImage->height  = WAP_ASCII_FONT_HEIGHT*3;
                ...
                //else if(result == 1)
                {
						...
						|| pCurNode->y2 <= g_nLCDOffset)   )
					{
                        ...
                        if(g_nViewMode == WAP_VIEW_MODE_WML)
                        {
                            WAP_Draw_Element_Image(pCurNode, 0, 0);       

                            x1 = 0;//g_pCurFocusNode->x1;
                            y1 = (pCurNode->y1 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
                            x2 = WAP_LCD_TOTAL_ADDRESS_X-1;
                            y2 = (pCurNode->y2 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
...
void Support_NoWrapping(void)
{
	...
	if(g_pCurFocusNode != NULL && g_pCurFocusNode->type == _ELEMENT_TYPE_KEY)
	{
			...
			|| g_pCurFocusNode->y2 <= g_nLCDOffset)   )
		{
            ...
            x1 = 0;//g_pCurFocusNode->x1;
            y1 = (g_pCurFocusNode->y1 - g_nLCDOffset) + WAP_HEAD_HEIGHT;
            x2 = WAP_LCD_TOTAL_ADDRESS_X-1;
            y2 = y1 + g_pCurFocusNode->font.height;
			//handset task에 display를 요청한다
			SKY_RefreshDisplayRect((BYTE *)GetScreenBuffer(), x1, y1, x2, y2);//WAP_Send_LCDBuffer_ToHS();
		}
	}
...
{
    EQS_FillRectangle(0, WAP_HEAD_HEIGHT - 2, WAP_LCD_TOTAL_ADDRESS_X-1, WAP_LCD_TOTAL_ADDRESS_Y-1, WAP_BASIC_BACK_COLOR);
}
