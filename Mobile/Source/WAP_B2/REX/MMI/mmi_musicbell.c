...
void musicbell_check(void)  //이 함수 수정금지
{
	if (g_fMusicbellOn)//ui.musicbell_on ) 
	{
		...
		MSG_HIGH("musicbell stop",0,0,0);
	}else
	{
		MSG_HIGH("musicbell off state",0,0,0);
	}
}
...
void musicbell_cmd(void)
{
	...
	{
		MSG_HIGH( "No UI cmd buffers",0,0,0);
	}
	else		
	{
        ...
        MSG_HIGH( " musicebell cmd 수행",0,0,0);
	}	
			...
			case Memory_File_Play:
    			...
    			switch(audio)
				{
                    case YAMAH_ma1:		//YAMAH일 경우만..
                        ...
                        SKY_PlayMusicBuffer((word*)mb_MusicBellPreBuffer);     //play music in buffer
#endif//#ifdef WAP_B1                        
                        break;

                    case YAMAH_mmf:		//YAMAH일 경우만..
#if (defined WAP_B1) || (defined WAP_B2)
                        SKY_PlayMusicBuffer((byte*)mb_MusicBellPreBuffer);     //play music in buffer
#endif  //#ifdef WAP_B1
...
void Musicbell_memplay(const char* Title, char *Result)
{
	...
	if(IsValid)
	{
		...
		//Message for user
		WAP_ClearScreen();
		WAP_SetTextAtLine(2, "연주 준비중입니다",WAP_TEXT_ALIGN_CENTER);
		WAP_DisplayMessage();
		//========================================================
