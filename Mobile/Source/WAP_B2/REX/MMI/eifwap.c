...
#include "eifwap.h"
#include "ui.h"
#include "..\MMI\SK\uiCommon.h" // for user defined types
#include "uistate.h"
...
char resultBuf[2048];						// Global variable for EIF
...
//void uping_rtt_result(uint32 RTT_result[]);
//void uping_rtt_store(uint32);
//void uping_time_store_value_free(void);
	...
	while(OutPutPtr != NULL)
	{
		...
		switch (OutPutPtr->ContentType)
		{
			case _ELEMENT_TYPE_KEY:
			{
				...
				if(strlen(resultBuf) == 0)
				{
					if (ParsedKey->nKey_number == WAP_KEY_SK_PREV)						//---이전
						...
						sprintf(resultBuf, "KEY(MENU):N,%s", ParsedKey->title);
				}
				else
				{
					if (ParsedKey->nKey_number == WAP_KEY_SK_PREV)						//---이전
						...
						sprintf(resultBuf, "%s\nKEY(MENU):N,%s", resultBuf, ParsedKey->title);
				}
			...
			case _ELEMENT_TYPE_LINK_TEXT:
			{
				...
				if(strlen(resultBuf) == 0)
					sprintf(resultBuf, "LINKTEXT:[%s]", ParsedLinkText->text);
				else
					sprintf(resultBuf, "%s\nLINKTEXT:[%s]", resultBuf, ParsedLinkText->text);
...
//				CLNTa_log(1,0, "[%s]", OptionLabels);
	...
	at_send_extended_text(resultBuf); // Result displaying	
...
DEPENDENCIES			입력 후 마지막 입력창에 포커스를 옮기도록 할 것.... 그리고 성공 메시지 보낼 것 
...
//	selectedString = selectedString + 7;	// selectedString 에서 "SELECT=" 문자열을 뛰어넘고 
...
{	
	...
	icmp_msg_to_send = dsm_new_buffer(DSM_SMALL_ITEM_SIZ);
	...
	dsm_pushdown(icmp_msg_ptr, NULL, ICMPLEN + sizeof(dword));	// send time을 실어 보낼 때 
...
//	extern WAP_Card *g_pCard;	// pointer of current card
	...
	if ((wapip != 0)&&(ui.call))
	{
		...
		else						// 현재 카드의 key에 대응되는 URL 가져오기....
		{
			...
			else
			{
				...
				sprintf(result, "*SKT*GETURL:%d,", number);
				strncpy(resultBuf, result, sizeof(resultBuf));
				strcat(resultBuf, EIF_url);
