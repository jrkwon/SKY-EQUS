...
static UINT32   maxsizes[NUM_FREE_LISTS] = {
  16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 0x10000, 0x20000
};
...
#ifdef WAP_USE_LEAK
	void* wip_malloc2(UINT32 size);
#endif//#ifdef WAP_USE_LEAK

////////////////////////////////////////////////////////////
//
//  Wip Array Memory API
//
////////////////////////////////////////////////////////////
#include "sisdecoder.h"

#define MAX_WIPARRAY_SIZE       10
#define MAX_WIPOFFSET_SIZE  5*1000
...
WIP_ARRAY_T WIP_ARRAY[MAX_WIPARRAY_SIZE];

static int ArrayOffsetSize = 0;
static int max_array_size  = 0;

void initWipArray(unsigned char* pStart, unsigned char* pEnd, int offset)
{
	int i;
	
	if( (pStart >= pEnd ) || !offset)
	{
#ifndef WIN32
		ERR_FATAL("Invalid paramemter",0,0,0);
#endif//win32
		return;
	}
	max_array_size  = (pEnd - pStart)/offset;
	ArrayOffsetSize = offset;

	if(max_array_size >= MAX_WIPARRAY_SIZE)
	{
		max_array_size = MAX_WIPARRAY_SIZE;
#ifndef WIN32
		MSG_MED("max_array_size[%d]",max_array_size,0,0);
#endif//win32
	}
	memset((void*)WIP_ARRAY, 0, sizeof(WIP_ARRAY) );
	for(i = 0; i < max_array_size; i++)
	{
		WIP_ARRAY[i].pWip = &pStart[offset*i];
	}
}

unsigned char* WipArrayAssign(unsigned int size)
{
	int i;

	if(size > ArrayOffsetSize)
	{
#ifndef WIN32
		ERR_FATAL("WipArrayAssign fail",0,0,0);
#endif
		return NULL;
	}
	...
	for(i = 0; i < max_array_size; i++)
	{
		if( WIP_ARRAY[i].inuse == 0)
		{
			WIP_ARRAY[i].inuse = 1;
			memset(WIP_ARRAY[i].pWip, 0, ArrayOffsetSize);
			MSG_HIGH("Array Memory is Assigned [i]",i,0,0);
			return WIP_ARRAY[i].pWip;
		}
	}
#ifndef WIN32
	ERR_FATAL("WipArrayAssign fail",0,0,0);
...
void WipArrayFreeAll(void)
{
	int i;

	MSG_HIGH("WipArrayFreeAll",0,0,0);
...
#ifdef FEATURE_WIPARRAY
  {
	  initWipArray(m_gObjBuf, m_gCodeBuf, MAX_WIPOFFSET_SIZE);
  }
#endif//wiparray  
  return 0;
}
 ...
 * Returns 0 if allocation fails.
 *
 * jrkwon 2001.06.12 --> SKY UI use only
 */
void *
...
wip_gcmalloc (UINT32 size)
{
  ...
  /* Searched all lists, but found no large enough chunk */
#ifdef FEATURE_WIPARRAY
  return WipArrayAssign(size);
#else
  CLNTa_log(1, 0, "!!!!No More Chunk!!!!");
  //ERR_FATAL("WIP_MALLOC error occured",0,0,0);
#endif//wiparray
  return 0;
...
#ifdef WAP_USE_LEAK
  if(!( pShareRAM < (unsigned char*)mem  && (unsigned char*)mem < pShareRAM + WIP_MALLOC_MEM_SIZE ))
  { 
	  
	  CLNTa_log(1,0,"test test test");
	  return;
  }

  if(blocknullfree) 
  {
	CLNTa_log(1,0," Free block");
	return; 
  }
#endif//#ifdef WAP_USE_LEAK
