...
000322 (KHN) Added support for proprietary script library functions.
...
#if     defined NO_GLOBAL_VAR
#include "userdata.h"
#endif

#if defined WTAI_NETWORK_COMMON || defined WTAI_GSM || defined WTAI_PDC || defined WTAI_IS_136
	#include "WTAI_IF.h"
#endif
...
enumErrorCode CallLibraryFunction( pstructSI si, UINT16 libIndex, UINT8 funcIndex )
{
    ...
    else if ((libIndex >= WTAI_LIB_INDEX_START) && (libIndex <= WTAI_LIB_INDEX_END))
    {
    	return WTAI_CallLibraryFunction( si, libIndex, funcIndex );
    }
#if defined WTAI_NETWORK_COMMON || defined WTAI_GSM || defined WTAI_PDC || defined WTAI_IS_136
	else if (libIndex >= WTAI_LIB_VCC && libIndex <= WTAI_LIB_PDC_NF )
	{
		UINT8   nbrOfArgs = 0;
		UINT16  timeCost = 1; 
		...
		{
			/* there is a corresponding WTAI function to be called via SDL over to the WTAI interface */
    ...
    else
    {
		...
		{
			CLNTa_log(1,0,"CallLibraryFunction] There is a library function");
	...
	if (Var_ConvertMethod( CONVERT_INT_FLOAT, op1, NULL ))
  {
		if (op1->type == typeFloat)
    {
		...
		if (op1->type == typeInteger)
		{
			...
			{	/* system dependant initialisation */
				lib_rand_seed( 161803398 );
			}
					...
					Var_ConvertMethod( CONVERT_STRINGS, op1, NULL ))
    {
			...
			stopIndex = MIN( MAX(op3->val.theInt, op3->val.theInt + startIndex), (INT32)op1->theStringLen );
				/* 000215,KHN: The "MAX(op3->val.theInt, op3->val.theInt + startIndex)" above
        								is a simplified overflow check. */
...
enumErrorCode Call_String_find( pstructSI si, UINT16 libIndex, UINT8 funcIndex )
{
	...
	if ( (VCR_OK == Var_Convert( op1, typeString )) && (VCR_OK == Var_Convert( op2, typeString ))) {
		if ((op1->theStringLen == 0) || (op2->theStringLen == 0)) {
	...
	if ( (VCR_OK == Var_Convert( src, typeString )) && (VCR_OK == Var_Convert( oldSub, typeString )) && (VCR_OK == Var_Convert( newSub, typeString ))) {
			...
			if (subPos != NULL) {
				...
				while (srcIndex < src->theStringLen) {
					...
					else { /* no more replacements => copy the rest of the src string to the new string */
						COPYSTRINGN( &(result->val.theString[dstIndex]), &(src->val.theString[srcIndex]),
...
#ifdef BLOCK
		///////////////////
		Uni2KSCString(str->val.theString, (unsigned char *)result_buffer);

		CLNTa_log(1,0,"Call_String_elementAt][%s]",result_buffer);
		///////////////////
#endif //#ifdef BLOCK
...
#ifdef BLOCK
						/////////////
						Uni2KSCString(result->val.theString, (unsigned char *)result_buffer);

						CLNTa_log(1,0,"Call_String_elementAt]result[%s]",result_buffer);
						/////////////
#endif //#ifdef BLOCK
					}
...
#ifdef WAPCORE_FOR_WARNING
	statusOK = (VCR_OK == Var_Convert( format, typeString ));
	if(statusOK)
#else
...
enumErrorCode Call_URL_getReferer( pstructSI si, UINT16 libIndex, UINT8 funcIndex )
{
  ...
  BYTE					*pbResult = b_MinRelativePath( pbBaseUrl, pbRefererUrl, FALSE ); /* we don't want the fragment (e.g. card name) */
...
#ifdef WAPCORE_FOR_WARNING
  allOk = Iana2Unicode_canConvert( charSet );
  if(allOk)
#else
...
*/
{
  ...
  /* invalid is the result if the function is not successful */
	...
	if ( (VCR_OK == Var_Convert( varName, typeString )) && (si->theUserAgent != NULL) )
	{
		...
		{
			if (WML_CheckVariableSyntax(varName->val.theString))
      {
...
*/
{
  ...
  /* invalid is the result if the function is not successful */
			...
			(si->theUserAgent != NULL) )
	{
      ...
      {
				/* success */
				Var_AssignBool( result, WML_SetVariable( si->theUserAgent, varName->val.theString, varVal->val.theString ));
			}
		}
	}
	...
	if (variable)
	{
		...
		switch (variable->type)
		{
			case typeInteger:
				...
				CLNTa_log(1,0,"NewWMLSvarFromPstructVar] Arg type is integer[%d]",result->intVal);
				break;
			...
			case typeString:
				...
				//CLNTa_log(1,0,"NewWMLSvarFromPstructVar] Arg type is String");
				...
				if (isMusicbellDownload)
				{
					if(strcmp(result->stringVal,"1")==0)
					{
						//result->stringVal = WIPAlloc(10);
						//w_strcpy(result->stringVal, "1");
						w_strcpy(attributeHistory, "1");
						CLNTa_log(1,0,"Attribute arg] Âø½Åº§·Î ÁöÁ¤");
					}
					else if(strcmp(result->stringVal,"2")==0)
					{
						//result->stringVal = WIPAlloc(10);
						//w_strcpy(result->stringVal, "2");
						w_strcpy(attributeHistory, "2");
						CLNTa_log(1,0,"Attribute arg] Âø½Åº§ ¾Æ´Ô");
					}
					else
					{
						CLNTa_log(1,0,"Attribute arg] broken:history[%s]",attributeHistory);
						result->stringVal = WIPAlloc(10);
						w_strcpy(result->stringVal, attributeHistory);
					}
				...
				//Uni2KSCString(result->stringVal, (unsigned char *)test_buffer);

				//CLNTa_log(1,0,"ÃÖÃÊ[%s]",test_buffer);
				break;

			case typeBool:
				result->boolVal = (BOOL) variable->val.theInt;
				CLNTa_log(1,0,"NewWMLSvarFromPstructVar] Arg type is boolean");
				break;
		}
			...
			case typeString:
				if ( (variable->stringVal != NULL) && (variable->stringLengthInBytes > 0) )
				{
				  BOOL		allOk = TRUE;
					INT32	nbrOfLetters = 0;
#ifdef WAPCORE_FOR_WARNING
					allOk = Iana2Unicode_canConvert( variable->stringIANAcharset );
					if(allOk)
#else
...
#endif //WAPCORE_FOR_WARNING
					{
  					nbrOfLetters = Iana2Unicode_calcLen((BYTE *) variable->stringVal, variable->stringIANAcharset, 
						...
						if ( nbrOfLetters > 0 )
						{
							...
							//Uni2KSCString(result->val.theString, (unsigned char *)test_buffer);

							//CLNTa_log(1,0,"ÃÖÁ¾][%s]",test_buffer);

							//CLNTa_log(1,0,"ÃÖÁ¾][%d]", w_strlen(result->val.theString));
...
*/
	...
	//CLNTa_log(1,0,"CallProprietaryLibraryFunction] ............");
	
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	//Convert result data to application format
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	for ( i=0 ; i < nbrOfArgs ; i++)
	{		
		var = RE_Pop( si->RE );
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction][type:%d].",var->type);
	...
	//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Before calling CLNTa_callWMLSLibFunc");

	returnVal = CLNTa_callWMLSLibFunc( si->invokeID, libIndex, funcIndex, argList, & resultLater );

	//CLNTa_log(1,0,"CallProprietaryLibraryFunction] resultLater is [%d]", resultLater);


	/////////////////////////////////////////////////////////////////////////////////////////////////////
	//Convert result data to core format
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	if (returnVal && (!resultLater))
	{
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Before convert result data to core format");
		var = NewPstructVarFromWMLSvar( returnVal );
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] After convert result data to core format");
	}

	/////////////////////////////////////////////////////////////////////////////////////////////////////
	//Free application allocated memory
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	if (returnVal)
	{
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Before free application allocated memory");
		WMLSvar_Delete( & returnVal, TRUE );
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] After free application allocated memory");
	}

	/* deallocation */
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	//Free core allocated memory for arguments
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	for ( i=0 ; i < nbrOfArgs ; i++)
	{
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Before Free core allocated memory for arguments");
		WMLSvar_Delete( & argList[i], FALSE );
	
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] After Free core allocated memory for arguments");
	}

	/////////////////////////////////////////////////////////////////////////////////////////////////////
	//Free core allocated memory for arguments list
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Before Free core allocated memory for arguments list");
	DEALLOC( &argList );
	//CLNTa_log(1,0,"CallProprietaryLibraryFunction] After Free core allocated memory for arguments list");
...
*/
	...
	if (var && !resultLater)
	{
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Direct Success Case");
		OpS_Push( si->RE->OpS, &var );
		return ERR_WAE_WMLS_NONE;
	}
	else if (!resultLater)
	{
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Direct Failure Case");
		return ERR_WAE_WMLS_LIB;
	}
	else
	{	/* i.e. resultLater == TRUE */
		//CLNTa_log(1,0,"CallProprietaryLibraryFunction] Separate Success Case");
...
enumErrorCode Lib_ProprietaryLibraryFunctionResponse( pstructSI si, WMLSvar **pResultVar )
{
	...
	//CLNTa_log(0, 0, "Lib_ProprietaryLibraryFunctionResponse]1111");
	...
	if (*pResultVar)
	{
		//CLNTa_log(0, 0, "Lib_ProprietaryLibraryFunctionRespons]2222");
		WMLSresultVar = NewPstructVarFromWMLSvar( *pResultVar );


		//CLNTa_log(0, 0, "Lib_ProprietaryLibraryFunctionResponse \n ±¸Á¶Ã¼[%x], ¸â¹ö[%x]", *pResultVar, (*pResultVar)->stringVal);
	...
	if (WMLSresultVar)
	{
		...
		//CLNTa_log(0, 0, "Lib_ProprietaryLibraryFunctionRespons]3333");
		return ERR_WAE_WMLS_NONE;
	}
	else
	{
