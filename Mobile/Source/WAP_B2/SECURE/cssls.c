...
#ifdef FOR_MS
	extern uc		ucVerifyDataOpen[1500];
	extern ul		ulVerifyDataLenOpen;
#endif
...
void close_wtls_session(void)
{
	...
	if(spSKT!=NULL)
	{
		ssls_clr_buf(spSKT);wip_free(spSKT);spSKT = NULL;
	}
}
...
)
{
		...
		us	usExpLen; uc ucExp[300];
		us	usModLen; uc ucMod[300];
...
//================================================================================
	 ...
	 /*==========================================================================	
	  ...
	  ==========================================================================*/	
		...
		//store handshake messages 
		memcpy(ucVerifyDataOpen + ulVerifyDataLenOpen,ucMsgBuf + REC_HEAD_LEN,(unsigned int)ulMsgBufSize);
					ulVerifyDataLenOpen += ulMsgBufSize;
			
		nRet = send_record(stpPaq, spSsls, ucMsgBuf, ulMsgBufSize + REC_HEAD_LEN, npErr,cpUrl, nMode);
...
{
	extern int clnt_wsp_send(unsigned char *ucpSendData, unsigned int nSendLen);
	ul ulHash;
    int nRet = 0;
	...
	nRet = clnt_wsp_send(spSsls->ucpBuffer,spSsls->ulBufSize); 
	if(nRet == -1)
	{
		return FAILURE;
	}
...
)
{
    ...
    if(ssls_str_buf(spSsls, ucpRec, ulRecLen) < 0) {
     ...
     /* store record hash */
    compAlrtHash(
        (char *)spSsls->ucpBuffer, (int)spSsls->ulBufSize, (char *)&ulHash
    );
	str_shash(spSsls, ulHash);

    /* send the record to wsp */

	/*===========================================================================================
	              UI task에서 처리하도록  command만 준다.
	 ===========================================================================================*/
	...
	else
	{
		...
		ui_cmd_ptr->hdr.cmd        = UI_WTLS_SEND_F;
