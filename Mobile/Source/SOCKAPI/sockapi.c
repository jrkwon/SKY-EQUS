...
#include "dssocket.h"
#include "uisockApi.h"
#include "dssnet.h"	//	khekim 01.05.25 :: dssnet_ppp_down_cb()
...
typedef	enum	
{
	...
	SOCK_PPPOPENING_S,			/* 1 */
	SOCK_PPPOPEN_S,				/* 2 */
	SOCK_UDPSOCK_OPENING_S,		/* 3 */
	SOCK_UDPSOCK_OPEN_S,		/* 4 */
	SOCK_TCPSOCK_OPENING_S,		/* 5 */
	SOCK_TCPSOCK_OPEN_S,		/* 6 */
	SOCK_TCPSOCK_REOPENING_S,	/* 7 */
	SOCK_CLOSING_S				/* 8 */	
}	SOCK_NETSTATE;
...
{	
	...
	switch (sockNetState)
	{
		...
		case	SOCK_TCPSOCK_OPEN_S:
			...
			break;
		case	SOCK_CLOSING_S:
			//khekim 01.05.25 :: TCP CLosing후 SOCK_SOCKET_CB_SIG 수신
			sock_requested_sig_mask &= ~(SOCK_SENDDATA_Q_SIG/* | SOCK_SOCKET_CB_SIG*/);
			//khekim 01.05.25 :: PPP CLosing후 SOCK_NET_CB_SIG 수신
			sock_requested_sig_mask |= SOCK_NET_CB_SIG;
...
#ifdef	FEATURE_SKT_GVM_ONLINE
				// khekim 01/04/25 
				// GVM download 다 받은 Endkey를 계속 누를 때, PPP Close가 완전히 되기 전에
				// Enter_browser mode에 진입함으로써 dssnet_ppp_down_cb()가 실행되지 않아
				// dssneti_set_netstate(DSSNET_CLOSED_STATE)가 설정되지 않으므로써
				// Socket 동작이 이후에 안되는 버그 수정
				dssnet_ppp_down_cb();
				dss_pppopen(appID, &errno);
#endif	//	#ifdef	FEATURE_SKT_GVM_ONLINE
				break;
		}
		if (errno == DS_EWOULDBLOCK) 
		{
			if(sock_wait_sig((long int)PERIOD_20SEC/*PERIOD_10SEC*/,  // khekim 01.05.25 :: PPP Open이 10초 넘어서 되는 경우 있음
...
// socketID may be lesser than 0x1000
...
)
{
	...
	{
		MSG_ERROR("sockApi_recvTCPData:DataSize0 Abnormal Released",0,0,0);
// khekim 01/02/22
...
)
{
	...
	{
		MSG_ERROR("sockApi_recvUDPData: DataSize 0",0,0,0);
	}
}
...
)
{
	...
	strcpy(aServerName, serverName);	//	save serverName
#endif	//	#ifdef	FEATURE_SOCKET_DNS_API
...
#else	//	#ifdef	FEATURE_SOCKET_DNS_API
	if (strcmp((char *)aServerName, serverName)!=0)
	{
...
{
	sint15 errno;
	struct sockaddr_in	*addr = &g_udpHostAddr;
	...
	if (state==DSS_ERROR)
	{
		switch(errno)
		{
			...
			case DS_ENETEXIST:
				...
				// khekim 01.05.25
				// PS Task의 동작이 느려서 PPP/TCP 모두 Close안되는 경우,
				// PS Task로부터 PPP Close되었다는 것을 직접 처리 하도록 수정
				// 이로써 dssneti_set_netstate(DSSNET_CLOSED_STATE)가 설정되고
				// TCP, PPP Close는 바로 수행됨
				dssnet_ppp_down_cb();
				sockApi_tcpClose(g_socketID);	/* khekim 01.05.25 :: PPP/TCP 모두 Close안되는 경우 있음 */
				sockApi_pppClose(g_appID);
				state = dss_close_netlib(g_appID, &errno);
				break;
		}
...
)
{
	...
	sockApi_setNetState(SOCK_CLOSING_S);
	sockApi_closeNetwork(oldSockNetState);
    // khekim 01.05.19 :: confirim socket released indication to UI
 //   SvrLinkReleased();
}
