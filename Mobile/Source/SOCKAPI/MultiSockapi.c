...
LOCAL FUNCTIONS
	...
	sint15	sockApi_recvUDPDatabyEvent(sint15 appID, sint15 socketID, byte* buf, uint16 len, 
			uint16 flags, struct sockaddr* from, uint16*	tolen)
	sint15	sockApi_closeNetwork(SOCK_NETSTATE	closedSockNetState)
...
typedef	enum	
{
	...
	SOCK_PPPOPENING_S,
	SOCK_PPPOPEN_S,
#ifndef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	SOCK_UDPSOCK_OPENING_S,
	SOCK_UDPSOCK_OPEN_S,
	SOCK_TCPSOCK_OPENING_S,
	SOCK_TCPSOCK_OPEN_S,
#endif	//	#ifndef	FEATURE_SKT_MULTISOCKAPI
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
sint15							g_nAppID[MAX_APPS];
sint15							g_nTCPSockID[MAX_TCP_SOCKS], g_nUDPSockID[MAX_UDP_SOCKS];
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
			memset((sint15 *)g_nAppID, DSS_ERROR, MAX_APPS);
			memset((sint15 *)g_nTCPSockID, DSS_ERROR, MAX_TCP_SOCKS);
			memset((sint15 *)g_nUDPSockID, DSS_ERROR, MAX_UDP_SOCKS);
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
			g_appID = DSS_ERROR;
...
sint8	sockApi_WaitSockSig
(
	uint32	nTimePeriod,
	uint32	signaledEventMask,
	uint32	requestedEvent
)
{
	uint32	signaledEvent;

	signaledEvent = sock_wait_sig(nTimePeriod, signaledEventMask, requestedEvent);

	if (signaledEvent==SOCK_CMD_Q_SIG)
	{
		// Close All Socket
		sockApi_closeNetworkbyReleaseCmd();
		return (DSS_ERROR);
	}
	else if (signaleEvent==SOCK_PS_TIMER_SIG)
	{
		// Close All Socket
		sockApi_closeNetworkbyReleaseCmd();
		return (DSS_ERROR);
	}
	else if ((requestedEvent&signaleEvent)!=0)
	{
		if (signaleEvent==SOCK_NET_CB_SIG)
		{
			return (DSS_SUCCESS);
		}
		else if (signaleEvent==SOCK_SOCKET_CB_SIG)
		{
			return (DSS_SUCCESS);
		}
#ifdef	FEATURE_SOCKET_DNS_API	//	khekim 00/11/14
		else if (signaleEvent==SOCK_DNS_CB_SIG)
		{
			return (DSS_SUCCESS);
		}
#endif	//	#ifdef	FEATURE_SOCKET_DNS_API
...
)
{
	...
	if (dss_pppopen(appID, &errno) == DSS_ERROR) 
	{
		...
		{
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/03
			if (DSS_ERROR==sockApi_WaitSockSig((uint32)PERIOD_10SEC, 
												(uint32)(SOCK_CMD_Q_SIG | SOCK_NET_CB_SIG  | SOCK_SOCKET_CB_SIG | SOCK_PS_TIMER_SIG),
												(uint32)(SOCK_CMD_Q_SIG | SOCK_PS_TIMER_SIG | SOCK_NET_CB_SIG)))
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/03
				if (DSS_ERROR==sockApi_WaitSockSig((uint32)PERIOD_5SEC, 
													(uint32)(SOCK_CMD_Q_SIG | SOCK_NET_CB_SIG  | SOCK_SOCKET_CB_SIG | SOCK_PS_TIMER_SIG),
													(uint32)(SOCK_CMD_Q_SIG | SOCK_PS_TIMER_SIG | SOCK_NET_CB_SIG)))
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
)
{
	...
	if (result == DSS_ERROR) 
	{
		switch (errno) 
		{
			...
			case DS_EWOULDBLOCK:
				...
				if (DSS_ERROR==sockApi_WaitSockSig((uint32)PERIOD_5SEC, 
													(uint32)(SOCK_CMD_Q_SIG | SOCK_NET_CB_SIG  | SOCK_SOCKET_CB_SIG | SOCK_PS_TIMER_SIG),
													(uint32)(SOCK_CMD_Q_SIG | SOCK_PS_TIMER_SIG | SOCK_SOCKET_CB_SIG)))
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
				if (sock_wait_sig((long int)PERIOD_5SEC, 
	...
	{
		if (sockApi_DeleteTCPSocket(socketID)==DSS_SUCCESS)
		{
			return (DSS_SUCCESS);
		}
		else
		{
			ERR("sockApi_tcpClose: sockApi_DeleteTCPSocket Failure", 0, 0, 0);
			return (DSS_ERROR);
		}	
	}
#endif	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
)
{
	...
	if (result==DSS_SUCCESS)
	{
		if (sockApi_DeleteUDPSocket(socketID)==DSS_SUCCESS)
		{
			return (DSS_SUCCESS);
		}
		else
		{
			ERR("sockApi_udpClose: sockApi_DeleteUDPSocket Failure", 0, 0, 0);
			return (DSS_ERROR);
		}	
	}
#endif	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
)
{
	...
	{
		if (g_nAppID[i]!=DSS_ERROR)
		{
			result = dss_close_netlib(g_nAppID[i], &errNo);
			g_nAppID[i] = DSS_ERROR;
		}
	}
	return (DSS_SUCCESS);
}

sint15 sockApi_CloseAllAppnPPP
(
	void
)
{
...
)
{
	...
	for(i=0 ; i<MAX_TCP_SOCKS ; i++)
	{
		if (g_nTCPSockID[i]!=DSS_ERROR)
		{
			sockApi_tcpClose(g_nTCPSockID[i]);
			g_nTCPSockID[i] = DSS_ERROR;
		}
	}
	for(i=0 ; i<MAX_UDP_SOCKS ; i++)
	{
		if (g_nUDPSockID[i]!=DSS_ERROR)
		{
			sockApi_udpClose(g_nUDPSockID[i]);
			g_nUDPSockID[i] = DSS_ERROR;
		}
	}
	return (DSS_SUCCESS);
}
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	if (sockApi_SaveSocket(socketID)==DSS_SUCCESS)
	{
		return (socketID);
	}
	else
	{
		MSG_ERROR("sockApi_socketOpen: Too Socket ID", 0, 0, 0);
		return (DSS_ERROR);
	}
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
	return (socketID); 
...
//	struct hostentTag	l_hostent;
	...
	// g_socketID should be set to close UDP socket successfully when receiving SOCKET_RELEASE_CMD by UI task
		if ((socketID=sockApi_socketOpen(appID, (byte)AF_INET, (byte)SOCK_DGRAM, (byte)IPPROTO_UDP))
				== DSS_ERROR) 
		{
			...
			==DSS_ERROR) 
		{
			...
			(void)sockApi_udpClose(socketID);
			return NULL;
		}
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/03
		if ((error==DS_EWOULDBLOCK) && 
			(DSS_SUCCESS==sockApi_WaitSockSig((uint32)PERIOD_10SEC, 
											(uint32)(SOCK_CMD_Q_SIG | SOCK_DNS_CB_SIG | SOCK_NET_CB_SIG  | SOCK_SOCKET_CB_SIG | SOCK_PS_TIMER_SIG),
											(uint32)(SOCK_CMD_Q_SIG | SOCK_PS_TIMER_SIG | SOCK_DNS_CB_SIG ))))
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
)
{
	...
	if (socketID<0x1000 || socketID>0x1FFF) 
	{
		...
		(void)sockApi_tcpClose(socketID);
		return (DSS_ERROR);
	}
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
sint15	sockApi_OpenNetlib
(
	void   (*sockApi_netCallback)(void *),         /* network callback function */
	void   (*sockApi_socketCallback)(void *)       /* socket callback function */
)
{
	sint15 errNo, nAppID;                                   /* error condition value */

	nAppID = dss_open_netlib(sockApi_netCallback, sockApi_socketCallback, &errNo);


	if (errNo==DSS_SUCCESS)	
	{
		if (sockApi_SaveApp(nAppID)==DSS_SUCCESS)
		{
			return (nAppID);
		}
		else
		{
			ERR("sockApi_OpenNetlib: Too App ID", 0, 0, 0);
			return (DSS_ERROR);
		}
	}
	else
...
)
{
	...
	// dss_open_netlib
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	s_nAppID = sockApi_OpenNetlib(&sockApi_netCallback, &sockApi_socketCallback);
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
	g_appID = dss_open_netlib(&sockApi_netCallback, &sockApi_socketCallback, &errno);
#endif	//	#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	...
	// dss_pppopen
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	if (sockApi_pppOpen(s_nAppID)==DSS_ERROR)
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	if (sockApi_getHostEntry(s_nAppID, serverName)==DSS_ERROR)
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	if ((s_nSockID=sockApi_tcpConnect(s_nAppID, g_hostEntry, portNo))==DSS_ERROR)
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	sockApi_enableRecvEvent(s_nSockID);	
#else	//	#ifndef	FEATURE_SKT_MULTISOCKAPI
...
)
{
	...
	//	sockNetState==SOCK_TCPSOCK_REOPENING_S
	...
	{
		result = sockApi_tcpClose(g_socketID); 
		sockApi_setNetState(SOCK_TCPSOCK_OPEN_S);
		sockApi_closeNetwork(sockNetState);
		SvrLinkResult(DSS_ERROR);
		return;
...
)
{
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
	if ((s_nSockID=sockApi_socketOpen(s_nAppID, (byte)AF_INET, (byte)SOCK_DGRAM, (byte)IPPROTO_UDP))
			== DSS_ERROR) 
#else	//	#ifdef	FEATURE_SKT_MULTISOCKAPI
...

	{
		...
		addr.sin_family = AF_INET;
		addr.sin_port   = dss_htons(portNo);	
		addr.sin_addr.s_addr = (((*g_hostEntry->h_addr_list[0])<<24) & 0xff000000 ) |
...
)
{
	...
	if (result==DSS_SUCCESS)
	{
		if (sockApi_DeleteApp(appID)==DSS_SUCCESS)
		{
			return (DSS_SUCCESS);
		}
		else
		{
			ERR("sockApi_CloseNetlib: sockApi_DeleteApp Failure", 0, 0, 0);
			return (DSS_ERROR);
		}	
	}
	return (result);
...
)
{
	...
	{
		case	SOCK_NULL_S:
			result = state sockApi_CloseAllApp();
			break;
		case	SOCK_PPPOPENING_S:
		case	SOCK_PPPOPEN_S:
			result = sockApi_CloseAllSocket();
			result = sockApi_CloseAllAppnPPP();
			break;
		default:
...
)
{
	...
	switch (closedSockNetState)
	{
		...
		case	SOCK_UDPSOCK_OPEN_S:
			if (g_socketID != DSS_ERROR)
			{
				(void)sockApi_udpClose(g_socketID); 
			}
			sockApi_pppClose(g_appID); 
			state = dss_close_netlib(g_appID, &errno);
			break;
		...
		case	SOCK_TCPSOCK_OPEN_S:
			if (g_socketID != DSS_ERROR)
			{
				(void)sockApi_tcpClose(g_socketID); 
			}
			sockApi_pppClose(g_appID); 
			state = dss_close_netlib(g_appID, &errno);
			break;
...
)
{
	if (sockNetState==SOCK_UDPSOCK_OPEN_S)
	{
		sockApi_sendUDPData(appID, socketID, 0, (struct sockaddr *)g_udpHostAddr, g_sizeofudpHostAddr);
	}
	else if (sockNetState==SOCK_TCPSOCK_OPEN_S)
	{
		sockApi_sendTCPData(appID, socketID);
	}
}
...
)
{
	if (sockNetState==SOCK_UDPSOCK_OPEN_S)
	{
		sockApi_recvUDPData(appID, socketID, 0, (struct sockaddr *)g_udpHostAddr, &g_sizeofudpHostAddr);
	}
	else if (sockNetState==SOCK_TCPSOCK_OPEN_S)
	{
		sockApi_recvTCPData(appID, socketID);
	}
}
...
)
{
	...
	if (sockNetState==SOCK_UDPSOCK_OPEN_S || sockNetState==SOCK_TCPSOCK_OPEN_S)
	{
		...
		if (mask & DS_WRITE_EVENT)
		{
			...
			sockApi_sendData(g_appID, g_socketID);
		}
		if (mask & DS_READ_EVENT)
		{
			MSG_HIGH("SOCKET_CB_SIG: DS_READ_EVENT", 0, 0, 0);
			sockApi_recvData(g_appID, g_socketID);
			sockApi_enableRecvEvent(g_socketID);	
		}
	}
}
...
#ifdef	FEATURE_SKT_MULTISOCKAPI	//	khekim 01/02/04
sint15	sockApi_SaveApp
(
	sint15	appID
)
{
	int		i;
	sint15	bFound = DSS_ERROR;
...
)
{
	...
	if ((type==SOCK_SGRAM) && (protocol==IPPROTO_TCP))
	{
		for(i=0 ; i<MAX_TCP_SOCKS ; i++)
		{
			if (g_nTCPSockID[i]==DSS_ERROR)
			{
				g_nTCPSockID[i] = socketID;
				bFound = DSS_SUCCESS;
				return (bFound);
			}
		}
...
sint15 sockApi_DeleteTCPSocket
(
	sint15	socketID
)
{
...
sint15 sockApi_DeleteUDPSocket
(
	sint15	socketID
)
{
