...
#-------------------------------------------------------------------------------
!IFDEF WAP2
WAP_DEF     = -DFEATURE_WAP -DTRGT_REX -DUSE_WIP_MALLOC -DUSE_PROPRIETARY_WMLS_LIBS -DWAPDORMANT -DFEATURE_STI_WAP
WAP_LOG     = -DLOG_ENABLE 
#-DVASTEST -DFEATURE_SKIPDATA
...
!IFDEF MLZ
SISLIB = $(WAP_DIR)\saf_ads_lib_010925_MLZ.a	#\sislib_arm.alf
!ELSE
SISLIB = $(WAP_DIR)\saf_ads_lib_010531_interw.a	#\sislib_arm.alf
!ENDIF

!IFDEF CHARACTER_BELL          
MMFLIB = $(WAP_DIR)\mmf_ads_lib_010917.a	#\mmflib_arm.alf
...
MESSAGE_OBJS =	$(TARGETDIR)\ppp_.o#
# 인터널 램관련 
# 메시지를 보시려면 해당 오브젝트 파일을 여기로 옮기고
# MODULE SPECIFIC RULES 에 가셔서 
# MESSAGE_OBJS에 대한 메시지 레벨의 결정해 주시면 됩니다.
# 이 작업은 반드시 로컬에서 해 주십시요 ^_^
# 위의 ppp_.o 파일은 ds_objs중 ppp_.o의 메시지를 보기위한 예입니다.
# youandi 01.11.01
...
$(MESSAGE_OBJS) :
	@echo ------------------------------------------------------------------
	@echo OBJECT $@
	$(CC) $(CFLAGS) $(DMSS_CFLAGS) $(MSG_FILE) $(OBJ_FILE) $(SLT_FILE)
	@echo ------------------------------------------------------------------

# 인터널 램과 관련되어 발생하는 
# 메시지를 보시려면 해당 오브젝트 파일을 MESSAGE_OBJS = #에 옮기고
# MESSAGE_OBJS의 DMSS_CFLAGS에 메시지 레벨의 결정해 주시면 됩니다.
# 이 작업은 반드시 로컬에서 해 주십시요 ^_^
# youandi 01.11.01
############################################################################
############################################################################
...
#	@echo ------------------------------------------------------------------
#	@echo OBJECT $@
#	$(CC) $(CFLAGS) $(DMSS_CFLAGS_DS) $(MSG_FILE) $(OBJ_FILE) $(SLT_FILE)
#	@echo ------------------------------------------------------------------
...
WAP_ROOT = $(TARGETDIR)\wapaddress.o \
...
MAKEFILE_NAME   = $(MAKEFILE)ADS.mak
MAKEFILE_BACKUP = $(MAKEFILE)ADS.bak
